...
static inline void run_timer_list(void)
{
	spin_lock_irq(&timerlist_lock);
        ...
        /* On the HH, we advance jiffies to its new value while asleep; in
           this case, we need to be careful to avoid running the main timer
           handling code over and over again, because it makes waking from
           standby slow.  We work around this problem by manually rescheduling
           all the timers if a large number of jiffies has elapsed since the
           last timer interrupt. */

#define LONG_JIFFIES_DELTA 6000

	if ((long)(jiffies - timer_jiffies) >= LONG_JIFFIES_DELTA) {
            int j, k;
            LIST_HEAD(alltimers);
            ...
            for (k = 0; k < TVR_SIZE; k++) {
                ...
                while (curr != head) {
                    ...
                    tmp = list_entry(curr, struct timer_list, list);
                    curr = curr->next;
                    list_add(&tmp->list, alltimers.prev);
                }
                INIT_LIST_HEAD(head);
            }

            for (j = 1; j < NOOF_TVECS; j++) {
            ...
            for (k = 1; k < NOOF_TVECS; k++)
                tvecs[k]->index = 
                    ((timer_jiffies + (1 << (TVR_BITS + TVN_BITS * 
                                             (k - 1))) - 1) /
                     (1 << (TVR_BITS + TVN_BITS * (k - 1)))) % TVN_SIZE;

            struct list_head *curr;
            curr = alltimers.next;
            while (curr != &alltimers) {
                struct timer_list *tmp;
