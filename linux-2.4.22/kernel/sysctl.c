...
#if defined(CONFIG_PPC_SONOS_ZP) || defined(CONFIG_CONNECTX_SONOS_ZP)
#define KERNEL_TEXT_START 0xc0000000
#define KERNEL_TEXT_END   0xc0180000
#elif defined(CONFIG_SH_SONOS_ZP)
#define KERNEL_TEXT_START 0x8c010000
#define KERNEL_TEXT_END   0x8c148000
#endif
...
#endif
#ifdef CONFIG_SONOS
	{KERN_BOOTSECTION,"bootsection", &bootsection, sizeof(int),
	0444, NULL, &proc_dointvec},
	{KERN_BOOTGENERATION,"bootgeneration", &bootgeneration, sizeof(int),
	0444, NULL, &proc_dointvec},
#endif
#ifdef __x86_64__
...
#if defined(CONFIG_PPC_SONOS_ZP) || defined(CONFIG_CONNECTX_SONOS_ZP)
	if ((table->proc_handler < (proc_handler *)KERNEL_TEXT_START)
		|| (table->proc_handler > (proc_handler *)KERNEL_TEXT_END)) {
		/* The proc_handler is invalid and a kernel oops will follow. Dump the contents of de (proc_dir_entry)
		   and table (ctl_table). See SWPBL-52377 */
		int i, len;
		char *pc;
		#define TMPBUFLEN 20
		char buffer[TMPBUFLEN], *p;
		memset(buffer, 0, TMPBUFLEN);
		printk("ASSERT: invalid proc handler found %p, table %p, de %p\n", table->proc_handler, table, de);
		printk("file %p, f_dentry %p, d_inode %p\n", file, file->f_dentry, file->f_dentry->d_inode);
		printk("Contents of *de\n");
		pc = (char *)de;
		for (i = 0; i < sizeof(*de); i++) {
			printk("%02x ", pc[i] & 0x000000ff);
		}
		printk("\nContents of *table\n");
		pc = (char *)table;
