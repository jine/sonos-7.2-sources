 ...
 *
 *	$Id: br_forward.c,v 1.8 2004/10/14 04:13:32 millington Exp $
 *
                                 ...
                                 struct net_bridge_port *p_outgoing)
{
    ...
    /* or if the carrier is missing */
    if ((!(p_outgoing->is_p2p))&&(!netif_carrier_ok(p_outgoing->dev)))
	return 0;
...
static int __dev_queue_wrapper_xmit(struct net_bridge_port *p, struct sk_buff *skb)
{
	...
	if ((!p->is_p2p || p->is_uplink) &&
	    br_mcast_is_management_header(skb->mac.ethernet)) {
...
static void __br_forward(struct net_bridge_port *to, struct sk_buff *skb)
{
	skb->dev = to->dev;
        ...
        // Forwarding is on by default in a diag build, but can be disabled
        // during the WiFi test.
        if (to->br->sonos_diag_bridge_forward)
#endif
        __br_forward_finish(to, skb);
}
