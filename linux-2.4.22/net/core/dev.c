...
#ifdef	__SONOS_LINUX_SH4__
		{
			/* SH4 has a single Ethernet MAC that connects to a switch.
			 * ethtool needs to get state for each port.
			 * To do that we pretend that each port is a separate device, eth0, eth1, ...
			 */
			int ret = 0;
			...
			if (ecmd.cmd == ETHTOOL_GSET) {
				...
				if (dev->do_ioctl) {
					...
					ifr.ifr_name[3] = save_dev;
					ret = dev->do_ioctl(dev, &ifr, cmd);
				}
ethtool_unlock:
				rtnl_unlock();
				dev_probe_unlock();
				if (!ret && copy_to_user(arg, &ecmd, sizeof(ecmd)))
					return -EFAULT;
				return ret;
			}
		}
