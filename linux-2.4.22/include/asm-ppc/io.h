...
#else /* Everyone else */
/* for example CONFIG_MPC8272ADS and CONFIG_ADS8266 */
...
extern void pci_idma1_read(u8 *, u8 *, int, int, int);
#endif

#if defined CONFIG_ADS8266
extern inline void _insb(volatile u8 *port, void *buf, int ns) {
        unsigned long pa = iopa((unsigned long)port);

        pci_idma1_read((u8 *)buf, (u8 *)pa, ns * sizeof(u8), sizeof(u8), 0);
}
#else
...
extern inline void _insw(volatile u16 *port, void *buf, int ns) {
        ...
        pci_idma1_read((u8 *)buf, (u8 *)pa, ns * sizeof(u16), sizeof(u16), 0);
}
#else
...
extern inline void _insl(volatile u32 *port, void *buf, int nl) {
        ...
        pci_idma1_read((u8 *)buf, (u8 *)pa, nl * sizeof(u32), sizeof(u32), 0);
}
#else
...
#if defined CONFIG_ADS8266
extern inline void *memcpy_fromio(unsigned dst, unsigned src, size_t count) {
