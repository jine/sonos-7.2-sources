...
static inline char *bh_kmap_irq(struct buffer_head *bh, unsigned long *flags)
{
	...
	/* XXX BT the following had been __cli() - so I think this is
	 * still okay...
	 */
	local_irq_save(*flags);

	/* XXX BT low latency patch took the following check out, I am
	 * putting it back in.  It was previously outside the __cli()
	 * which I am pretty sure is wrong because it would, I believe,
	 * lead to an unbalanced local_irq_restore() in bh_kunmap_irq later
	 */
