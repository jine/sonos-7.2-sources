	...
	int flags, char *dev_name, void * data)
{
	...
	mode_t mode = FMODE_READ; /* we always need it ;-) */
        struct super_block *read_super_result;
...
restart:
	...
	insert_super(s, fs_type);
        read_super_result = fs_type->read_super(s, data, 
                                                flags & MS_VERBOSE ? 1 : 0);
        if (!read_super_result) {
            error = -EINVAL;
            goto Einval;
        } else if (IS_ERR(read_super_result)) {
            error = PTR_ERR(read_super_result);
            goto Einval;
        }
