...
static int cifs_calculate_signature(const struct smb_hdr * cifs_pdu, const char * key, char * signature)
{
	...
	MD5Update(&context,key,CIFS_SESSION_KEY_SIZE+16);
        cFYI(1,("Calculating signature of %d bytes from 0x%x",
	...
	__u32 * pexpected_response_sequence_number)
{
	...
	if((le16_to_cpu(cifs_pdu->Flags2) & SMBFLG2_SECURITY_SIGNATURE) == 0) 
		return rc;
...
/*	cifs_dump_mem("what we think it should be: ",what_we_think_sig_should_be,16); */
	...
	if(memcmp(server_response_sig, what_we_think_sig_should_be, 8)) {
            ...
            cifs_dump_mem("what we think it should be: ",what_we_think_sig_should_be,8);
            cifs_dump_mem("what it was: ",server_response_sig,8);
        	return -EACCES;
        }
	else
		return 0;
