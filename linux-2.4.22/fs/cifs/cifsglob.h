...
/*
 * One of these for each open instance of a dir
 */
struct cifsDirInfo {
    /* data from most recent server transaction */
    char find_data[CIFS_MAX_MSGSIZE];   /* directory enumeration result */
    int unicode;                        /* unicode results? */
    int unixSearch;                     /* UNIX-style results? */
    unsigned int searchCount;           /* number of results */
    int endOfSearch;                    /* no more results available? */
    unsigned int currentRecord;         /* record currently processed */
    unsigned int currentRecordOffset;
    
    __u16 netfid;                       /* search handle */
};
