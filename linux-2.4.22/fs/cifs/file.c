...
cifs_open(struct inode *inode, struct file *file)
{
	...
	if ((file->f_flags & O_ACCMODE) == O_RDONLY)
		desiredAccess = (READ_CONTROL | FILE_READ_ATTRIBUTES | FILE_READ_EA | FILE_READ_DATA);
...
cifs_closedir(struct inode *inode, struct file *file)
{
	int rc = 0;
	int xid;
        struct cifsDirInfo *pSMBDirStruct =
            (struct cifsDirInfo *) file->private_data;
	...
	xid = GetXid();
        ...
        if (pSMBDirStruct) {
                if (!pSMBDirStruct->endOfSearch) {
                        struct cifs_sb_info *cifs_sb;
                        struct cifsTconInfo *pTcon;

                        cifs_sb = CIFS_SB(inode->i_sb);
                        pTcon = cifs_sb->tcon;       
                        CIFSFindClose(xid, pTcon, pSMBDirStruct->netfid);
                }
...
#endif
                if((int)current_read_size<0) {
                    cERROR(1,("Negative read size %d (cifs_sb->rsize %d)",
                             current_read_size, cifs_sb->rsize));
                }
		rc = -EAGAIN;
		while(rc == -EAGAIN) {
			if ((open_file->invalidHandle) && (!open_file->closePend)) {
...
fill_in_inode(struct inode *tmp_inode,
          const FILE_DIRECTORY_INFO * pfindData, int *pobject_type)
{
    ...
    cifsInfo->cifsAttrs = le32_to_cpu(pfindData->ExtFileAttributes);
    AllocationSize = le64_to_cpu(pfindData->AllocationSize);
    EndOfFile = le64_to_cpu(pfindData->EndOfFile);
	...
	cFYI(0,
	     ("CIFS FFIRST: Attributes came in as 0x%x",
          cifsInfo->cifsAttrs));
    if (cifsInfo->cifsAttrs & ATTR_DIRECTORY) {
		*pobject_type = DT_DIR;
    ...
    tmp_inode->i_size = EndOfFile;

	/* 512 bytes (2**9) is the fake blocksize that must be used */
	/* for this calculation, even though the reported blocksize is larger */
    tmp_inode->i_blocks = (512 - 1 + tmp_inode->i_size) >> 9;

    if (AllocationSize < EndOfFile)
...
unix_fill_in_inode(struct inode *tmp_inode,
           const FILE_UNIX_INFO * pfindData, int *pobject_type)
{
	struct cifsInodeInfo *cifsInfo = CIFS_I(tmp_inode);
	cifsInfo->time = jiffies;
    ...
    NumOfBytes = le64_to_cpu(pfindData->NumOfBytes);
    EndOfFile = le64_to_cpu(pfindData->EndOfFile);
    tmp_inode->i_size = EndOfFile; 
...
static int
cifs_filldir(struct qstr* qstring, const FILE_DIRECTORY_INFO * pfindData,
	     struct file *file, filldir_t filldir, void *direntry)
{
	...
	construct_dentry(qstring, file, &tmp_inode, &tmp_dentry);
...
static int
cifs_filldir_unix(struct qstr* qstring, const FILE_UNIX_INFO * pUnixFindData, 
                  struct file *file, filldir_t filldir, void *direntry)
{
	struct inode *tmp_inode;
	struct dentry *tmp_dentry;
	...
	rc = filldir(direntry, qstring->name, qstring->len,
...
cifs_readdir(struct file *file, void *direntry, filldir_t filldir)
{
    ...
    struct cifs_sb_info *cifs_sb;
    struct cifsTconInfo *pTcon;
    struct cifsDirInfo *cifsDir = NULL;
    ...
    /* First enumeration result is always "." */
    if (file->f_pos == 0) {
    ...
    /* Second enumeration result is always ".." */
    if (file->f_pos == 1) {
    ...
    if (file->f_pos == 2) {
        ...
        if (file->private_data) {
            cifsDir = (struct cifsDirInfo*)file->private_data;
            if (!cifsDir->endOfSearch)
                CIFSFindClose(xid, pTcon, cifsDir->netfid);
        } else {
            cifsDir = kmalloc(sizeof(*cifsDir), GFP_KERNEL);
            if (!cifsDir) {
                rc = -ENOMEM;
                goto cleanup;
            }

            file->private_data = cifsDir;
        }

        /* call the server to get some results */
        full_path = build_wildcard_path_from_dentry(file->f_dentry);
        rc = CIFSFindFirst(xid, pTcon, full_path, 
                           (FILE_DIRECTORY_INFO *)cifsDir->find_data,
                           &findParms, cifs_sb->local_nls,
                           &cifsDir->unicode, &cifsDir->unixSearch);
        if (rc == 0) {
            /* OK, we've started up the enumeration successfully */
            cifsDir->searchCount = findParms.SearchCount;
            cifsDir->endOfSearch = findParms.EndofSearch;
            cifsDir->currentRecordOffset = 0;
            cifsDir->currentRecord = 0;
            cifsDir->netfid = findParms.SearchHandle;

            renew_parental_timestamps(file->f_dentry);
        } else {
            /* terminate the enumeration, but don't return error */
            cifsDir->endOfSearch = 1;
            rc = 0;
            goto cleanup;
        }
    }
    ...
    /* now do the main part of the algorithm, fetching data again from the
       server as necessary. */
    for (;;) {
        const FILE_DIRECTORY_INFO* pfindData = 0;
        while (cifsDir->currentRecord < cifsDir->searchCount) {
            pfindData = 
                (const FILE_DIRECTORY_INFO*)&cifsDir->
                find_data[cifsDir->currentRecordOffset];
            
            /* handle the record */
            struct qstr qstring;
            char string_val[2048];
            if (!cifsDir->unixSearch) {
                if (cifsDir->unicode) {
                    qstring.len = cifs_strfromUCS_le(string_val,
                                                     (const wchar_t*)
                                                     pfindData->FileName,
                    ...
                    memcpy(rgch, pfindData->FileName, 
                           le32_to_cpu(pfindData->FileNameLength));
                    qstring.len = cifs_convertn(string_val, 
                                                pfindData->FileName, 
                                                2047, 
                                                le32_to_cpu(pfindData->FileNameLength),
                                                cifs_sb->local_nls, 
                ...
                qstring.name = string_val;

                if (0 != strcmp(qstring.name, ".") && 
                    0 != strcmp(qstring.name, "..")) {
                    if (cifs_filldir(&qstring, 
                                     pfindData,
                                     file, 
                                     filldir,
                                     direntry)) {
                        /* kernel is not ready for any more results */
                        goto cleanup;
                    }
                }
            } else {	/* UnixSearch */
                const FILE_UNIX_INFO *pfindDataUnix =
                    (const FILE_UNIX_INFO *) pfindData;
                if (cifsDir->unicode) {
                    qstring.len = cifs_strfromUCS_le(string_val,
                                                     (const wchar_t*)
                    ...
                    qstring.len = cifs_convert(string_val, 
                            pfindDataUnix->FileName, MAX_PATHCONF,
                            cifs_sb->local_nls, CIFS_TO_LOCAL);
                }
            ...
            file->f_pos++;
            cifsDir->currentRecord++;
            cifsDir->currentRecordOffset += 
                le32_to_cpu(pfindData->NextEntryOffset);
        }

        if (cifsDir->searchCount == 0)
            cifsDir->endOfSearch = 1;

        /* if there is more data to fetch from the server, do so now */
        if (!pfindData || cifsDir->endOfSearch)
            break;
        else {
            ...
            if (!cifsDir->unixSearch) {
                resume_name_length = 
                    le32_to_cpu(pfindData->FileNameLength);
                if (resume_name_length > bufsize - 64) {
                    cFYI(1,("Illegal resume file name length %d",
                            resume_name_length));
                    rc = -ENOMEM;
                    goto cleanup;
                }
                search_resume_name = kmalloc(resume_name_length, GFP_KERNEL);
                memcpy(search_resume_name,
                       pfindData->FileName, 
                       resume_name_length);
            } else {
                const FILE_UNIX_INFO *pfindDataUnix = 
                ...
                if (cifsDir->unicode) {
                    ...
                    resume_name_length = i + 2;
                } else {
                    resume_name_length = strnlen(pfindDataUnix->FileName,
                                                 bufsize - 63);
                }
                if (resume_name_length > bufsize - 64) {
                    cFYI(1,("Illegal resume file name length %d",
            ...
            rc = CIFSFindNext(xid, pTcon, 
                              (FILE_DIRECTORY_INFO *)cifsDir->find_data,
                              &findNextParms, cifsDir->netfid,
                              search_resume_name, resume_name_length, 
                              pfindData->FileIndex,
                              &cifsDir->unicode, &cifsDir->unixSearch);
            kfree(search_resume_name);
            if (rc == 0) {
                cifsDir->searchCount = findNextParms.SearchCount;
                cifsDir->endOfSearch = findNextParms.EndofSearch;
                cifsDir->currentRecordOffset = 0;
                cifsDir->currentRecord = 0;
            } else {
                /* terminate the enumeration, but don't return error */
...
/* Rincon added support for direct READ operations in order to prevent disk
   cache thrash during long-running music playback. */
int cifs_direct_fileIO(int rw, struct file *file, struct kiobuf *iobuf, 
                   unsigned long blocknr, int blocksize)
{
	int rc = -EACCES;

	unsigned int read_size = iobuf->length; /* size of data buffer */
	unsigned int read_offset = blocknr; /* offset into the file */
	unsigned int total_read;

	unsigned int current_offset = iobuf->offset; /* offset into page */
	unsigned int current_ix = 0;
	char *current_read_data;
	...
	while (total_read < read_size) {
		...
		while(rc == -EAGAIN) {
			...
			/* TML NOTE: CIFS in 2.6 has some pretty narly code that deals
			   with "large" buffers. Not sure what problem they were working
			   around, but we may need to account for it. */
			rc = CIFSSMBRead(xid, pTcon,
				 open_file->netfid,
				 current_read_size, read_offset,
				 &bytes_read, &smb_read_data);
			if ((rc == -EAGAIN) && (smb_read_data != NULL)) {
				cifs_buf_release(smb_read_data);
				smb_read_data = NULL;
			}
		}
		if ((rc < 0) || (smb_read_data == NULL) || (bytes_read == 0)) {
			break;
		}
		...
		/* copy data to iobuf */
		pSMBr = (struct smb_com_read_rsp *)smb_read_data;
		current_read_data = smb_read_data + 4 /* RFC1001 hdr */ +
					le16_to_cpu(pSMBr->DataOffset);
		current_read_size = bytes_read;
		while (current_read_size > 0) {
			struct page *page = iobuf->maplist[current_ix];
			char *mapped_data;
			unsigned int mapped_amt;

			if (current_ix >= iobuf->nr_pages || !page) {
			    rc = -EFAULT;
			    break;
			}

			page_cache_get(page);
			mapped_data = kmap(page);

			mapped_data += current_offset;
			mapped_amt = PAGE_SIZE - current_offset;
			if (mapped_amt > current_read_size) {
				mapped_amt = current_read_size;
				current_offset += mapped_amt; /* can write more into this page */
			} else {
				current_offset = 0; /* reset offset to start a new page */
				current_ix++;
			}

			memcpy(mapped_data, current_read_data, mapped_amt);

			flush_dcache_page(page);
			kunmap(page);
			page_cache_release(page);

			current_read_data += mapped_amt;
			current_read_size -= mapped_amt;
		}

		if (smb_read_data != NULL) {
			cifs_buf_release(smb_read_data);
			smb_read_data = NULL;
		}
...
/*	.readpages = cifs_readpages, */
	...
	.direct_fileIO = cifs_direct_fileIO
};
