...
struct smb_vol {
	...
	mode_t file_mode;
	mode_t dir_mode;
        unsigned use_ntlmssp; /* Per volume security configuration */
	int rw:1;
	int retry:1;
	int intr:1;
	unsigned int rsize;
...
cifs_demultiplex_thread(struct TCP_Server_Info *server)
{
	...
	sprintf(current->comm,"cifsd");
        ...
        /* REVIEW: CIFS FS added PF_MEMALLOC to the process flags in order to
           prevent re-entrant calls into CIFS FS to free dirty pages and flush
           them to disk.  We are not doing this because we don't want to 
           run out of RAM in here, and we never make disk writes to CIFS 
           servers anyhow. */
			...
			if (temp[0] == (char) RFC1002_SESSION_KEEP_ALIVE) {
				...
				if ((length != sizeof (struct smb_hdr) - 1)
					...
					for (total_read = 0; 
						...
						if ((length == -ERESTARTSYS) || (length == -EINTR)) {
							cFYI(1,("cifsd thread killed"));
							do_exit = 1;
							break;
						} else if (length == 0) {
							...
							cERROR(1, ("Error while receiving: %d", -length));
							do_reconnect = 1;
							break;
						}
					}
...
/* this is basically strsep looking for a "," delimiter where ",," escapes */
static char *
cifs_tokenize_mount_options(char **stringp)
{
	char *stringp_orig = *stringp;

	if (!stringp_orig)
		return 0;

	char *pch_write = stringp_orig;
	char *pch_read = stringp_orig;
...
cifs_parse_mount_options(char *options, const char *devname, struct smb_vol *vol)
{
	...
	while ((data = cifs_tokenize_mount_options(&options)) != NULL) {
		if (!*data)
			continue;
	   ...
	   char *mount_data, const char *devname)
{
	...
	if (existingCifsSes) {
		...
		if (!rc){
			...
			pSesInfo->linux_uid = volume_info.linux_uid;
                        pSesInfo->use_ntlmssp = volume_info.use_ntlmssp;
	...
	if (!rc) {
		if((volume_info.rsize) && (volume_info.rsize + MAX_CIFS_HDR_SIZE < srvTcp->maxBuf))
			...
			/* SONOS: always start with the max buffer size */
			// cifs_sb->rsize = srvTcp->maxBuf - MAX_CIFS_HDR_SIZE; /* default */
			cifs_sb->rsize = CIFS_MAX_MSGSIZE;
		}
	...
	if (rc) {
		...
		if (tcon->ses->capabilities & CAP_UNIX) {
		    if(CIFSSMBQFSUnixInfo(xid, tcon, cifs_sb->local_nls)) {
                        /* SONOS: If this fails, turn off UNIX extentensions
                            rather than failing the mount later */
                        tcon->ses->capabilities &= ~CAP_UNIX;
                    }
                }

		/* SONOS: If the server doesn't support large read/writes,
		          reduce the buffer sizes to the negotiated values */
	      ...
	      const struct nls_table *nls_codepage)
{
	...
	/* SONOS: Large read/write capability query to see if we can use
	          max sized buffers */
	...
	if (ses->capabilities & CAP_UNICODE) {
		...
		if (user != NULL) { 
                    bytes_returned = cifs_convert(bcc_ptr,
                                                  user, 200, nls_codepage, 
                                                  CIFS_TO_REMOTE);               
		    bcc_ptr += bytes_returned;
		}
		*bcc_ptr = 0;
		bcc_ptr++;
		if (domain == NULL) {
			...
			bcc_ptr += strlen("CIFS_LINUX_DOM") + 1;
                        ...
                        bytes_returned = cifs_convert(bcc_ptr,
                            domain, 64, nls_codepage, CIFS_TO_REMOTE);
                        bcc_ptr += bytes_returned;
			*bcc_ptr = 0;
			bcc_ptr++;
		}
...
/* We look for obvious messed up bcc or strings in response so we do not go off
   ...
   terminating last Unicode string in response  */
          ...
          /* last string is not always null terminated (for e.g. for Windows XP & 2000) */
					...
					len = cifs_strnlen(bcc_ptr, 1024, nls_codepage);
					if (((long) bcc_ptr + len) - (long)
					    pByteArea(smb_buffer_response)
					    <= BCC(smb_buffer_response)) {
						ses->serverOS = cifs_kcalloc(len + 1, GFP_KERNEL);
						cifs_convert(ses->serverOS, bcc_ptr, len,
                                                             nls_codepage, CIFS_TO_LOCAL);
						...
						cifs_convert(ses->serverNOS, bcc_ptr, len,
                                                             nls_codepage, CIFS_TO_LOCAL);
						bcc_ptr += len;
						bcc_ptr[0] = 0;
						bcc_ptr++;
						...
						cifs_convert(ses->serverDomain, bcc_ptr, len,
                                                             nls_codepage, CIFS_TO_LOCAL);
						bcc_ptr += len;
						bcc_ptr[0] = 0;
						bcc_ptr++;
				 ...
				 const struct nls_table *nls_cp)
{
	...
	ses->serverOS = cifs_kcalloc((4 * len) + 2 /* trailing null */, GFP_KERNEL);
	...
	if (ses->serverNOS)
            kfree(ses->serverNOS);
	ses->serverNOS = cifs_kcalloc((4 * len) + 2 /* trailing null */, GFP_KERNEL);
	...
	ses->serverDomain = cifs_kcalloc((4 * len) + 2, GFP_KERNEL);
...
/* SONOS: We don't support NTLMV2, so comment this out */
#if 0
				if(SecurityBlob2->NegotiateFlags & NTLMSSP_NEGOTIATE_NTLMV2)
					*pNTLMv2_flag = TRUE;
#endif
/* SONOS: Also don't negotiate signing since it doesn't work */
#if 0
...
#endif
				if (smb_buffer->Flags2 &= SMBFLG2_UNICODE) {
                                        remaining_bytes = BCC(smb_buffer_response) - pSMBr->resp.SecurityBlobLength;
					if ((long) (bcc_ptr) % 2) {
						--remaining_bytes;
						bcc_ptr++;
					}
		                        rc = decode_unicode_ssetup(&bcc_ptr, 
                                                                  remaining_bytes,
					                          ses,
                                                                  nls_codepage);
                                        cFYI(1,("Native OS: %s  Native LAN: %s  Primary Domain: %s",ses->serverOS,ses->serverNOS,ses->serverDomain));
				} else {	/* ASCII */
					len = cifs_strnlen(bcc_ptr, 1024, nls_codepage);
					if (((long) bcc_ptr + len) - (long)
					    pByteArea(smb_buffer_response)
		...
		*(bcc_ptr + 2) = 0;
		...
		if (domain == NULL) {
			...
			SecurityBlob->NegotiateFlags |=
			    NTLMSSP_NEGOTIATE_DOMAIN_SUPPLIED;
                        bytes_returned = cifs_convert(bcc_ptr, domain, 63,
                                                      nls_codepage,
                                                      CIFS_TO_REMOTE);
			SecurityBlob->DomainName.Length = bytes_returned;
			SecurityBlob->DomainName.MaximumLength =
			    cpu_to_le16(SecurityBlob->DomainName.Length);
			SecurityBlob->DomainName.Buffer =
			    cpu_to_le32(SecurityBlobLength);
		...
		if (user == NULL) {
			...
			SecurityBlob->UserName.MaximumLength = 0;
                        ...
                        bytes_returned = cifs_convert(bcc_ptr, user, 63,
                                                      nls_codepage,
                                                      CIFS_TO_REMOTE);
			SecurityBlob->UserName.Length = bytes_returned;
			SecurityBlob->UserName.MaximumLength =
			    cpu_to_le16(SecurityBlob->UserName.Length);
			SecurityBlob->UserName.Buffer =
			    cpu_to_le32(SecurityBlobLength);
	 ...
	 const struct nls_table *nls_codepage)
{
	...
	if (ses->capabilities & CAP_UNICODE) {
		...
		length = cifs_convert(bcc_ptr,tree,100,nls_codepage,CIFS_TO_REMOTE);
		bcc_ptr += length + 1;
	}
	strcpy(bcc_ptr, "?????");
	bcc_ptr += strlen("?????");
	...
	if ((rc == 0) && (tcon != NULL)) {
		...
		if (smb_buffer->Flags2 &= SMBFLG2_UNICODE) {
			...
			length = cifs_strnlen(bcc_ptr, 1024, nls_codepage);
			if (((long) bcc_ptr + length) -
			    ...
			    BCC(smb_buffer_response)) {
                            ...
                            cifs_convert(tcon->nativeFileSystem, bcc_ptr,length,
                                         nls_codepage, CIFS_TO_LOCAL);
			}
			/* else do not bother copying these informational fields */
		}
					   ...
					   struct nls_table * nls_info)
{
	...
	if (!rc) {
                /* SONOS: Lion workaround: if we are forcing NTLMSSP, assume
                     UNIX extensions are not supported.  Lion claims to support
                     them but doesn't, causing an error on reconnect */
		if(pSesInfo->use_ntlmssp && 
                    pSesInfo->server->secType == RawNTLMSSP)
			pSesInfo->server->capabilities &= (~CAP_UNIX);
		...
		cFYI(1,("Security Mode: 0x%x Capabilities: 0x%x Time Zone: %d",
			...
			pSesInfo->server->timeZone));
                cFYI(1,("Security Type: %d",pSesInfo->server->secType));
