...
CIFSSMBNegotiate(unsigned int xid, struct cifsSesInfo *ses)
{
	...
	if (extended_security || ses->use_ntlmssp) {
		pSMB->hdr.Flags2 |= (SMBFLG2_EXT_SEC | SMBFLG2_ERR_STATUS);
                cFYI(1,("Using extended security 0x%08x",pSMB->hdr.Flags2));
        }
	...
	if (rc == 0) {
                cFYI(1,("In negotiation"));
		server->secMode = pSMBr->SecurityMode;	
                if(ses->use_ntlmssp)
		    server->secType = RawNTLMSSP; /* BB override default for NTLMv2 or krb*/
                else
		...
		} else if (pSMBr->EncryptionKeyLength != 0)
			rc = -EIO;

		/* BB might be helpful to save off the domain of server here */

                /* SONOS: Ignore the extended security flag unless we asked
                   for it to be set */
		if (pSMBr->hdr.Flags2 & SMBFLG2_EXT_SEC &&
                     (extended_security || ses->use_ntlmssp)) {
...
DelFileRetry:
	...
	if (pSMB->hdr.Flags2 & SMBFLG2_UNICODE) {
		...
		name_len *= 2;
                ...
                name_len = cifs_convert(pSMB->fileName, fileName,
                                        530, nls_codepage, CIFS_TO_REMOTE);
		name_len++;	/* trailing null */
	}
...
RmDirRetry:
	...
	if (pSMB->hdr.Flags2 & SMBFLG2_UNICODE) {
		...
		name_len *= 2;
                ...
                name_len = cifs_convert(pSMB->DirName, dirName,
                                        530, nls_codepage, CIFS_TO_REMOTE);
		name_len++;	/* trailing null */
	}
...
MkDirRetry:
	...
	if (pSMB->hdr.Flags2 & SMBFLG2_UNICODE) {
		...
		name_len *= 2;
                ...
                name_len = cifs_convert(pSMB->DirName, name,
                                        530, nls_codepage, CIFS_TO_REMOTE);
		name_len++;	/* trailing null */
	}
...
renameRetry:
	...
	/* protocol requires ASCII signature byte on Unicode string */
		...
		name_len2 *= 2;	/* convert to bytes */
                ...
                name_len = cifs_convert(pSMB->OldFileName, fromName,
                                        530, nls_codepage, CIFS_TO_REMOTE);
		name_len++;	/* trailing null */
		pSMB->OldFileName[name_len] = 0x04;  /* 2nd buffer format */
                name_len2 = cifs_convert(&pSMB->OldFileName[name_len + 1], toName, 530,
                                         nls_codepage, CIFS_TO_REMOTE);
...
createSymLinkRetry:
	...
	if (pSMB->hdr.Flags2 & SMBFLG2_UNICODE) {
		...
		name_len *= 2;
                ...
                name_len = cifs_convert(pSMB->FileName, fromName,
                                        530, nls_codepage, CIFS_TO_REMOTE);
		name_len++;	/* trailing null */
	}
	...
	if (pSMB->hdr.Flags2 & SMBFLG2_UNICODE) {
		...
		name_len_target *= 2;
                ...
                name_len_target = cifs_convert(data_offset, toName,
                                               530, nls_codepage, CIFS_TO_REMOTE);
		name_len_target++;	/* trailing null */
	}
...
createHardLinkRetry:
	...
	if (pSMB->hdr.Flags2 & SMBFLG2_UNICODE) {
		...
		name_len *= 2;
                ...
                name_len = cifs_convert(pSMB->FileName, toName,
                                        530, nls_codepage, CIFS_TO_REMOTE);
		name_len++;	/* trailing null */
	}
	pSMB->ParameterCount = 6 + name_len;
	...
	if (pSMB->hdr.Flags2 & SMBFLG2_UNICODE) {
		...
		name_len_target *= 2;
                ...
                name_len_target = cifs_convert(data_offset, fromName,
                                               530, nls_codepage, CIFS_TO_REMOTE);
		name_len_target++;	/* trailing null */
	}
...
querySymLinkRetry:
	...
	if (pSMB->hdr.Flags2 & SMBFLG2_UNICODE) {
		...
		name_len *= 2;
                ...
                name_len = cifs_convert(pSMB->FileName, searchName,
                                        530, nls_codepage, CIFS_TO_REMOTE);
		name_len++;	/* trailing null */
	}
			...
			if (pSMBr->hdr.Flags2 & SMBFLG2_UNICODE) {
				...
				cifs_strfromUCS_le(symlinkinfo,
					...
					name_len, nls_codepage);
                                ...
                                cifs_convert(symlinkinfo, (char *) &pSMBr->hdr.Protocol + 
                                             pSMBr->DataOffset, 
	...
	if (rc) {
		...
		/* could also validate reparse tag && better check name length */
				...
				if (pSMBr->hdr.Flags2 & SMBFLG2_UNICODE) {
					...
					cifs_convert(symlinkinfo,reparse_buf->LinkNamesBuf + 
                                                     reparse_buf->TargetNameOffset, 
	...
	if (rc) {
		...
		if ((pSMBr->ByteCount < 40) || (pSMBr->DataOffset > 512)) 
			rc = -EIO;	/* bad smb */
                        ...
                        size_t toCopy = pSMBr->ByteCount > 
                            sizeof(FILE_ALL_INFO) ? sizeof(FILE_ALL_INFO) : 
                            pSMBr->ByteCount;
			memcpy((char *) pFindData,
			       (char *) &pSMBr->hdr.Protocol +
			       pSMBr->DataOffset, toCopy);
                        size_t toSet = toCopy < sizeof(FILE_ALL_INFO) ? 
                            sizeof(FILE_ALL_INFO) - toCopy : 0;
                        if (toSet)
                            memset(((char*)pFindData) + toCopy,
                                   0,
                                   toSet);
		} else
		    rc = -ENOMEM;
...
findFirstRetry:
	...
	pSMB->SearchAttributes =
	    cpu_to_le16(ATTR_READONLY | ATTR_HIDDEN | ATTR_SYSTEM |
			ATTR_DIRECTORY);
	// SONOS: use the minimum sized entry to work around OSX Yosemite, which will prematurely
	// set EndOfSearch on hitting the SearchCount even if there are more entries
	pSMB->SearchCount = cpu_to_le16(tcon->ses->server->maxBuf/sizeof(FILE_BASIC_INFO));
	//pSMB->SearchCount = cpu_to_le16(16);
	...
	if (rc) {		/* BB add logic to retry regular search if Unix search rejected unexpectedly by server */
		...
		cFYI(1, ("FindFirst got SearchCount=%d EndOfSearch=%d",
...
/* level 257 SMB_ */
	...
	if(name_len < CIFS_MAX_MSGSIZE) {
		memcpy(pSMB->ResumeFileName, resume_file_name, name_len);
                memset(pSMB->ResumeFileName + name_len, 0, 2);
		pSMB->ByteCount += name_len;
	}
	...
	if (rc) {
		...
		response_data =
		    ...
		    le16_to_cpu(pSMBr->DataOffset);
		memcpy(findData, response_data, le16_to_cpu(pSMBr->DataCount));
		cFYI(1, ("FindNext got SearchCount=%d EndOfSearch=%d",
			findParms->SearchCount, findParms->EndofSearch));
	}
	if (pSMB)
		cifs_buf_release(pSMB);
...
getDFSRetry:
	...
	if (ses->capabilities & CAP_UNICODE) {
		...
		name_len *= 2;
                ...
                name_len = 
                    cifs_convert(pSMB->RequestFileName, searchName,
                                 530, nls_codepage, CIFS_TO_REMOTE);
		name_len++;	/* trailing null */
	}
				...
				if (pSMBr->hdr.Flags2 & SMBFLG2_UNICODE) {
					cifs_strfromUCS_le(*targetUNCs,
						(wchar_t *) temp, name_len, nls_codepage);
                                        ...
                                        cifs_convert(*targetUNCs,temp, name_len,
                                                     nls_codepage, CIFS_TO_LOCAL);
				}
