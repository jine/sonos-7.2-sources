 ...
 */
	...
	printk(KERN_ALERT "pid = %d pgrp = %d\n", tsk->pid, tsk->pgrp);
...
out_of_memory:
	...
	if (user_mode(regs)) {
		...
		for_each_task(q) {
			if (q->mm == tsk->mm) {
				/*
				 * This is mostly the same as what
				 * oom_kill_task does, except we always send
				 * SIGKILL.
				 */
				q->counter = 5 * HZ;
				q->flags |= PF_MEMALLOC | PF_MEMDIE;
				force_sig(SIGKILL, q);
			}
		}
	...
	/* We killed the faulting process.  Our business is done. */
	goto bad_area;
