/* $Id: setup.c,v 1.16.26.1 2006/01/06 21:21:51 tober Exp $
 *
...
#ifdef CONFIG_SONOS
extern int nand_init_success;			// poorly used to determine es2 or a3 model
#endif
	...
	/* Try to probe the memory size */
       {
		...
		switch(v) {
			case 2:
				printk("16 Megabytes of SDRAM detected\n");
				memory_end = memory_start + 0x1000000;
				break;
			case 3:
				printk("32 Megabytes of SDRAM detected\n");
				memory_end = memory_start + 0x2000000;
				break;
			default:
				printk("Unable to detect SDRAM size\n");
		}
	}
#endif /*SONOS*/
...
#ifdef CONFIG_SONOS
setup_mv_zoneplayer( int model )
{
		sh_mv.mv_hw_zoneplayer = model;
		sh_mv.mv_hw_handheld = 0;
}

setup_mv_handheld( int model )
{
		sh_mv.mv_hw_zoneplayer = 0;
		sh_mv.mv_hw_handheld = model;
}
#endif
...
void __init setup_arch(char **cmdline_p)
{
	...
	MOUNT_ROOT_RDONLY = 1;
	RAMDISK_FLAGS = 0x00004000;
	ORIG_ROOT_DEV = 0x00000000;
	LOADER_TYPE = 0x00000001;
#ifdef CONFIG_SONOS
	INITRD_START = 0x01000000;
	INITRD_SIZE = 0x00340000;
#else
	INITRD_START = 0x02000000;
	INITRD_SIZE = 0x00220000;
#endif
	...
	parse_cmdline(cmdline_p, mv_name, &mv, &mv_io_base, &mv_mmio_enable);
        ...
        memcpy( saved_command_line,CONFIG_CMDLINE,COMMAND_LINE_SIZE );
...
#define PFN_PHYS(x)	((x) << PAGE_SHIFT)
	...
	if (LOADER_TYPE && INITRD_START) {
		if (INITRD_START + INITRD_SIZE <= ((max_low_pfn << PAGE_SHIFT)+PAGE_OFFSET)) {
	...
	if (&__rd_start != &__rd_end) {
		printk("Found a linked-in ramdisk @ %p - %p\n",&__rd_start,&__rd_end);
...
#ifdef CONFIG_SONOS
	if( MACH_ZONEPLAYER_A3 || MACH_ZONEPLAYER_ES2 )
		seq_printf(m,"Zoneplayer - model: %s\n",
			MACH_ZONEPLAYER_A3 ? "A3" : "ES2" );
	if( MACH_HANDHELD_A3 || MACH_HANDHELD_ES2 )
		seq_printf(m,"Handheld - model: %s\n",
			MACH_HANDHELD_A3 ? "A3" : "ES2" );
#endif
