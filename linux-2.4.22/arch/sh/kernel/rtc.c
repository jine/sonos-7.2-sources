...
void sh_rtc_gettimeofday(struct timeval *tv)
{
 ...
 again:
	do {
		...
		/*
		 * We're working around a silly hardware bug here.  The
		 * SH-4 manuals says that the RTC counts 1/64ths of a second.
		 * In fact, it counts 1/128ths of a second, but it rolls
		 * the seconds at 64/128.  The problem is, the CF bit is
		 * based on the 128/128 wraparound.  When we read a 64, we
		 * don't actually know whether the seconds have been updated
		 * yet or not.  So we just spin for an entire tick.  Yuck.
		 */
		cf_bit |= (sec128 == RTC_BIT_INVERTED);
#endif
	} while (cf_bit != 0);
