/* $Id: time.c,v 1.9.26.1 2006/01/06 21:21:51 tober Exp $
 *
...
#define CONFIG_START_TMU1	1
#define TMU1_TCR_INIT	0x0027	/* Ext. Tclk, rising edge; interrupt on */
#else
...
/*
 * Ticks at the codec sample rate
 */

static unsigned long ctr[2] = { 0,0 };

static unsigned long do_audio_sample_ctr(void)
{
	if(ctrl_inw(TMU1_TCR) & 0x100) { /* Check UNF bit */
		ctr[0]++;
		ctrl_outw( (ctrl_inw(TMU1_TCR)&~0x100),TMU1_TCR );
	}
	ctr[1] = 0xffffffff - ctrl_inl(TMU1_TCNT);

	return( ctr[1] );
}

unsigned long  get_audio_sample_ctr( unsigned long *retval ) 
{
	unsigned long flags;
...
EXPORT_SYMBOL(get_audio_sample_ctr);
...
#ifdef CONFIG_SONOS_ZP
	/* audio clock under flow? */
	if( ctrl_inw(TMU1_TCR)&0x100 ) {
		do_audio_sample_ctr();			/* handle the overflow */
	}

	/* see if there was a zero underflow */
	if( ctrl_inw( TMU0_TCR )&0x100 ) {
#endif
