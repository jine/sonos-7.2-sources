...
static struct serial_state rs_table[] = {
	...
	{ 0,     0, PROFF_SCC1, SIU_INT_SCC1,   0,   (0 | NUM_IS_SCC) }, /* SCC1 ttyS0 */
	{ 0,     0, PROFF_SCC4, SIU_INT_SCC4,   0,   (3 | NUM_IS_SCC) }, /* SCC4 ttyS1 */
};
#else
static struct serial_state rs_table[] = {
	/* UART CLK   PORT          IRQ      FLAGS  NUM   */
...
#endif /*CONFIG_PPC_SONOS*/
#if USE_SMC2
	{ 0,     0, PROFF_SMC2, SIU_INT_SMC2,   0,    1 },    /* SMC2 ttyS1 */
#endif
#ifndef CONFIG_PPC_SONOS
	{ 0,     0, PROFF_SCC2, SIU_INT_SCC2,   0, (0 | NUM_IS_SCC) },    /* SCC2 ttyS2 */
	{ 0,     0, PROFF_SCC3, SIU_INT_SCC3,   0, (3 | NUM_IS_SCC) },    /* SCC3 ttyS3 */
#endif
};
#endif
...
/* console allocated data structures */
static uint   cons_rx_dpaddr;
static uint   cons_tx_dpaddr;
static cbd_t *cons_tx_bdp;
...
static _INLINE_ void receive_chars(ser_info_t *info)
{
	...
	for (;;) {
		...
		*/
		 ...
		 */
		/*bdp->cbd_sc |= BD_SC_EMPTY;*/
                /*XXX BT set BD_SC_EMPTY last*/
		bdp->cbd_sc &= ~(BD_SC_BR | BD_SC_FR | BD_SC_PR | BD_SC_OV);
                eieio();
                bdp->cbd_sc |= BD_SC_EMPTY;

                /* Get next BD.  */
                (bdp->cbd_sc&BD_SC_WRAP) ? bdp = info->rx_bd_base : ++bdp;
	}
...
#ifdef SERIAL_DEBUG_INTR
	printk("rs_interrupt_single(%d, %x)...", idx, events);
#endif
#ifdef modem_control
...
#define RELEVANT_IFLAG(iflag) (iflag & (IGNBRK|BRKINT|IGNPAR|PARMRK|INPCK))
	...
	m8260_cpm_setbrg(idx, baud_rate);
...
static void rs_8xx_put_char(struct tty_struct *tty, unsigned char ch)
{
	...
	*((char *)__va(bdp->cbd_bufaddr)) = ch;
	...
	bdp->cbd_sc |= BD_SC_READY;
        ...
        /* Get next BD.  */
        (bdp->cbd_sc&BD_SC_WRAP) ? bdp = info->tx_bd_base : ++bdp;
...
static void begin_break(ser_info_t *info)
{
	...
	num = PORT_NUM(info->state->smc_scc_num);
	if (!(info->state->smc_scc_num & NUM_IS_SCC)) {
		if (num == 0) {
...
static int inline line_info(char *buf, struct serial_state *state)
{
	...
	ret = sprintf(buf, "%d: uart:%s port:%X irq:%d",
		      state->line,
		      (!(state->smc_scc_num & NUM_IS_SCC)) ? "SMC" : "SCC",
 ...
 */
static void serial_console_write(struct console *con, const char *buf,
				unsigned count)
{
    int c;
    char *p;
    ...
    ser = rs_table + con->index;

    if ((info = (ser_info_t *)ser->info) != NULL) {
        bdp = info->tx_cur;
        bdbase = info->tx_bd_base;
    } else {
        bdbase = (cbd_t *)&immap->im_dprambase[cons_tx_dpaddr];
        if( (bdp = cons_tx_bdp) == NULL )
            bdp = bdbase;
    }

    while( ((c = MIN(count,TX_BUF_SIZE)) > 0) ) {
        ...
        if( (p = strchr(buf,'\n')) != 0 ) {
            ...
            if( c == TX_BUF_SIZE ) {
                --c; needCR = 0;    // do crlf pair next time
            }
        }
        ...
        /* Get next BD.  */
        (bdp->cbd_sc&BD_SC_WRAP) ? bdp = bdbase : ++bdp;

        if( info )
            info->tx_cur = (cbd_t *)bdp;
        else
            cons_tx_bdp = (cbd_t *)bdp;
    }
}

static kdev_t serial_console_device(struct console *c)
{
	/* XXX BT this had been TTYAUX_MAJOR, which is the callout (/dev/cua)
	 * device.  That's wrong, console should always use the normal device.
	 */
 ...
 *	Register console.
 */
void __init console_8xx_init(void)
{
#ifdef CONFIG_PPC_SONOS
...
uint rs_8xx_rx_bfr( int sc_flag )
{
    int j;
    uint dp_addr;
    uint mem_addr;
    ...
    for ( j = 0; j < (RX_NUM_FIFO-1); ++j,++bdp ) {
         bdp->cbd_bufaddr = __pa(mem_addr);
         bdp->cbd_sc = BD_SC_EMPTY | sc_flag;
         mem_addr += RX_BUF_SIZE;
    }
    bdp->cbd_bufaddr = __pa(mem_addr);
    bdp->cbd_sc = BD_SC_WRAP | BD_SC_EMPTY | sc_flag;

    return( dp_addr );
}

uint rs_8xx_tx_bfr( int sc_flag )
{
    int j;
    uint dp_addr;
    uint mem_addr;
    ...
    for ( j = 0; j < (TX_NUM_FIFO-1); ++j,++bdp ) {
         bdp->cbd_bufaddr = __pa(mem_addr);
         bdp->cbd_sc = sc_flag;
         mem_addr += TX_BUF_SIZE;
    }
    bdp->cbd_bufaddr = __pa(mem_addr);
    bdp->cbd_sc = (BD_SC_WRAP | sc_flag);
    return( dp_addr );
}

uint rs_8xx_set_flg( uint dp_addr, int nentries, int sc_flag )
{
    int j;
    volatile cbd_t   *bdp;
    volatile immap_t *immap = immr;
...
/*	io->iop_pparc |= 0x00020000;
	...
	io->iop_psorc &= 0xFFFDFFFF;
	io->iop_psord &= 0xFFFFFFFA;
	io->iop_psord |= 0x00000002;

	io->iop_pdirc &= 0xFFFDFFFF;
	io->iop_pdird &= 0xFFFFFFFC;
	io->iop_pdird |= 0x00000002;*/
#endif
...
#if defined(CONFIG_SCC2_UART) || defined(CONFIG_SCC3_UART)
	...
	*/
	 ...
	 * BRG4 to SCC3.
	 */
	immap->im_cpmux.cmx_scr &= ~0x000000ff;
	...
	for (i = 0, state = rs_table; i < NR_PORTS; i++,state++) {
		...
		if (info) {
			...
			state->info = (struct async_struct *)info;
                        ...
                        dp_addr = (i == CONFIG_SERIAL_CONSOLE_PORT) ? 
                            rs_8xx_set_flg( cons_rx_dpaddr, RX_NUM_FIFO, BD_SC_INTRPT ) :
                            rs_8xx_rx_bfr( BD_SC_INTRPT );
			...
			if (!(state->smc_scc_num & NUM_IS_SCC)) {
				...
				scp = &immap->im_scc[idx];
				sup = (scc_uart_t *)&immap->im_dprambase[state->port];
				sup->scc_genscc.scc_rbase = dp_addr;
			}

                        dp_addr = (i == CONFIG_SERIAL_CONSOLE_PORT) ? 
                            rs_8xx_set_flg( cons_tx_dpaddr, TX_NUM_FIFO, BD_SC_INTRPT ) :
                            rs_8xx_tx_bfr( BD_SC_INTRPT );
			...
			*/
			m8260_cpm_setbrg(idx, baud_table[baud_idx]);
			...
			*/
			...
			if (i == CONFIG_SERIAL_CONSOLE_PORT) {
                            if (state->smc_scc_num & NUM_IS_SCC) {
				scp->scc_gsmrl |= SCC_GSMRL_ENT | SCC_GSMRL_ENR;
			    } else {
...
static int __init serial_console_setup(struct console *co, char *options)
{
	...
	if (ser->smc_scc_num & NUM_IS_SCC) {
		scp = &immap->im_scc[idx];
		sup = (scc_uart_t *)&immap->im_dprambase[ser->port];
	} else {
		sp = &immap->im_smc[idx];
	...
	*(ushort *)(&immap->im_dprambase[PROFF_SMC2_BASE]) = PROFF_SMC2;
	 ...
	 */
        ...
        cons_rx_dpaddr = rs_8xx_rx_bfr( 0 );
        cons_tx_dpaddr = rs_8xx_tx_bfr( 0 );
	...
	if (ser->smc_scc_num & NUM_IS_SCC) {
		...
		*/
		sup->scc_genscc.scc_rbase = cons_rx_dpaddr;
		sup->scc_genscc.scc_tbase = cons_tx_dpaddr;
		sup->scc_genscc.scc_rfcr = CPMFCR_GBL | CPMFCR_EB;
		sup->scc_genscc.scc_tfcr = CPMFCR_GBL | CPMFCR_EB;
		...
		*/
		...
		/* Send the CPM an initialize command.
		*/
		cp->cp_cpcr = mk_cr_cmd(CPM_CR_SCC1_PAGE, CPM_CR_SCC1_SBLOCK, 0, CPM_CR_INIT_TRX) | CPM_CR_FLG;
		while (cp->cp_cpcr & CPM_CR_FLG);
		 ...
		 */
		scp->scc_gsmrh = 0x60;
		scp->scc_gsmrl = (SCC_GSMRL_MODE_UART | SCC_GSMRL_TDCR_16 | SCC_GSMRL_RDCR_16);
		...
		/* Set up the baud rate generator.
		*/
		m8260_cpm_setbrg(idx, baud_table[bidx]);
		...
		*/
		...
		/* Set up the uart parameters in the parameter ram.
		*/
		up->smc_rbase = cons_rx_dpaddr;
		up->smc_tbase = cons_tx_dpaddr;
		up->smc_rfcr = CPMFCR_GBL | CPMFCR_EB;
		up->smc_tfcr = CPMFCR_GBL | CPMFCR_EB;
