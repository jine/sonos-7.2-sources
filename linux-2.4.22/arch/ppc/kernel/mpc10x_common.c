...
#if defined CONFIG_8272 || defined CONFIG_ADS8266
                // XXX BT I'm pretty sure the person who originally wrote
                // this was confused between IORESOURCE_IO (PCI IO space,
                // which is an x86-ism that exists for ISA-compatibility and
                // is today very seldom used and IORESOURCE_MEM without
                // IORESOURCE_PREFETCH ("non-prefetcable memory address space)
                // which is what all sane modern implementations use for IO
                // registers
...
/* XXX BT don't do the one for the prefetchable memory, we never use it
 * and having it requires making a whole mess of PTEs for it and that takes
 * up a ton of space for no good reason...
 */
#if 0
		   ...
		   uint phys_eumb_base)
{
	...
	switch (current_map) {
		...
		case MPC10X_MEM_MAP_826x:
			setup_indirect_pci(hose,
					   MPC10X_MAP826x_CNFG_ADDR,
					   MPC10X_MAP826x_CNFG_DATA);
			break;
#endif
		default:
	...
	early_read_config_dword(hose,0,PCI_DEVFN(0,0),PCI_VENDOR_ID,&host_bridge);
#if 0
	printk("hose->cfg_addr is 0x%lx\n",(unsigned long)hose->cfg_addr);
	printk("hose->cfg_data is 0x%lx\n",(unsigned long)hose->cfg_data);
	printk("after early_config host_bridge is 0x%lx\n", host_bridge);
#endif
	switch (host_bridge) {
...
#if defined CONFIG_ADS8266
		case MPC10X_BRIDGE_8266:
#endif
#if defined CONFIG_8272 
		case MPC10X_BRIDGE_8272:
