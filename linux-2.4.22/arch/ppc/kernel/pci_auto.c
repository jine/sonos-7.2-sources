...
int __init pciauto_bus_scan(struct pci_controller *hose, int current_bus)
{
	...
	for (pci_devfn = 0; pci_devfn < 0xff; pci_devfn++) {
		...
		if (vid != 0xffff) {
			/*XXX BT moved this logic from up above so we don't
			 *wind up spending extra time considering sub-functions
			 *on devices that don't even exist, which appear to
			 *always return all-ones for all config reads
			 */
			if (!PCI_FUNC(pci_devfn))
				found_multi = header_type & 0x80;
			early_read_config_dword(hose,
