...
#ifdef CONFIG_PCI
#ifdef CONFIG_8272
extern void m826x_find_bridges(void);
#endif
#ifdef CONFIG_ADS8266
extern void pci_idma1_init(void);
extern void m826x_find_bridges(void);
#endif
#endif /* for CONFIG_PCI */
...
/*XXX BT the following is inexcuseable and unnecessary*/
#if 0
...
m8260_restart(char *cmd)
{
	...
	/* The following is sort of borrowed from uboot's reboot routine*/
        /* Interrupts and MMU off */
	unsigned long msr;
        __asm__ __volatile__ ("mfmsr    %0":"=r" (msr):);

        msr &= ~(MSR_ME | MSR_EE | MSR_IR | MSR_DR);
        __asm__ __volatile__ ("isync ; mtmsr    %0"::"r" (msr));
...
m8260_init_IRQ(void)
{
	...
	#if defined CONFIG_MPC8272ADS || defined CONFIG_ADS8266
	void mpc826xpci_intr(int , void *, struct pt_regs *);
	#endif
...
#ifdef CONFIG_PCI
#if defined CONFIG_MPC8272ADS || defined CONFIG_ADS8266
        for ( i = 0 ; i < NR_PCI_INTS ; i++ )
                irq_desc[NR_SIU_INTS + i].handler = &ppc826xpci_pic;
#endif
#endif
...
#ifdef CONFIG_PCI
	 /* after set the SIU interrupt priority reg and 
	 * CPM interupt priority reg
	 * here will set the external interrupt control reg to sure which kind of signal will effect as a interrupt signal
	 */
#ifndef CONFIG_PPC_SONOS
	immr->im_intctl.ic_siexr &= ~(1 << (14 - (INT_PCI_TO_SIU - SIU_INT_IRQ1))); /* level sensitive */
	m826xpci_pic_init();
	request_8xxirq(INT_PCI_TO_SIU, mpc826xpci_intr, 0, "PCI cascade", NULL);
#endif /*SONOS*/
	/*enable_irq(INT_PCI_TO_SIU);*/ /* XXX BT superfluous to the point of incorrectness*/
#endif
}
...
m8260_find_real_end_of_memory(void)
{
	bd_t	*binfo;
...
m8260_find_end_of_memory(void)
{
	...
	end = m8260_find_real_end_of_memory();
...
/////////////
#if defined CONFIG_8272 || defined CONFIG_ADS8266
	/* the first map will map 256M space*/
	// io_block_mapping(0x80000000, 0x80000000, 0x10000000, _PAGE_IO);
	io_block_mapping(0xa0000000, 0xa0000000, 0x10000000, _PAGE_IO);
	io_block_mapping(0xf0000000, 0xf0000000, 0x10000000, _PAGE_IO);
#ifdef CONFIG_SONOS_DIAGS
        { void * pptr;
          pptr=early_get_page();
          io_block_mapping(0xb0000000, __pa(pptr), 0x1000, _PAGE_IO);
        }
#endif
#else
	io_block_mapping(0xf0000000, 0xf0000000, 0x10000000, _PAGE_IO);
...
#ifdef CONFIG_SONOS_DIAGS
	/* See if we're supposed to reserve RAM for our evil purposes */
	unsigned long real_mem_size, mem_size;
	
	real_mem_size = m8260_find_real_end_of_memory();
	mem_size      = m8260_find_end_of_memory();
	
	if (mem_size != real_mem_size) {
		io_block_mapping(mem_size, mem_size, real_mem_size - mem_size, _PAGE_IO);
	}
#endif
	      ...
	      unsigned long r6, unsigned long r7)
{
	...
	/*XXX BT the following had been set to functions above which did
	 *nothing except save and restore the "RTC" time.  On architectures
	 *which lack an RTC, these function pointers should be 0.
	 */
