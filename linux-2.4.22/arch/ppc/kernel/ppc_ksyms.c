...
extern int actl_ledtimerdisable;
extern int actl_leddata[];
...
EXPORT_SYMBOL(m8260_port_set);
EXPORT_SYMBOL(m8260_port_read);
#endif

#ifdef CONFIG_PPC_SONOS
EXPORT_SYMBOL(audioctl_set_leds);
EXPORT_SYMBOL(actl_leddata);
EXPORT_SYMBOL(actl_ledcur);
EXPORT_SYMBOL(led_pwm_state);
EXPORT_SYMBOL(actl_ledtimerdisable);
EXPORT_SYMBOL(audioctl_ledtimer);
#endif
