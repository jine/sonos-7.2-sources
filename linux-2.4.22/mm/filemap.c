...
static ssize_t generic_file_direct_IO(int rw, struct file * filp, char * buf, size_t count, loff_t offset)
{
	...
	retval = -EINVAL;
        ...
        // SONOS - we use direct I/O for non block oriented CIFS io
	...
	while (count > 0) {
		...
		if (retval)
			break;
                ...
                // SONOS - we also use direct io for non block based offsets
                retval = do_call_directIO(rw, filp, iobuf, offset+progress, blocksize);
#else
