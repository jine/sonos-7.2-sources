...
#define LED_AMBER_PORT 0
#define LED_AMBER_PIN 13
#define LED_GREEN_PORT 0
#define LED_GREEN_PIN 10
#define LED_WHITE_PORT 3
#define LED_WHITE_PIN 30

#endif /*PPC_SONOS*/

struct manufacturing_data_page sys_mdp = {
	0, /*not MDP_MAGIC so the user can tell this is boilerplate*/
	MDP_VENDOR_RINCONNETWORKS,
#ifdef CONFIG_SH_SONOS_ZP
	MDP_MODEL_ZP,
#else
#ifdef CONFIG_PPC_SONOS_ZP
	MDP_MODEL_WEMBLEY,
#else
...
#endif /*PPC_ZP*/
#endif /*SH_ZP*/
	MDP_UNKNOWN, /*submodel*/
	MDP_UNKNOWN, /*revision*/
	MDP_EMPTY_SERIAL, /*serial*/
	MDP_REGION_USCA, /*region*/
	0, /*reserved*/
	MDP_DEFAULT_COPYRIGHT,
	0, /* flags */
	0, /*hwfeatures*/
	0, /*ch11spurimmunitylevel*/
	"", /*reserved2*/
	{ "" /*u_reserved*/ }
};
...
#ifdef CONFIG_SONOS
	/* If condition is true, compiler attempts to determine the sizeof an array of len -1, which
	 * causes a compilation failure.  If condition is false, it attempts to determine the sizeof
	 * an array of len 1, which is fine, but it doesn't instantiate any code, and the (void)
	 * prevents unused warnings.  This is a standard kernel macro in 2.6+.
	 */
#ifndef BUILD_BUG_ON
 ...
 */
	...
	extern void *sh_bios_get_mdp(void);
	struct manufacturing_data_page *mdp = (struct manufacturing_data_page*)sh_bios_get_mdp();
	memcpy( (void *)&sys_mdp,(void *)mdp,sizeof(struct manufacturing_data_page) );
#endif
	lock_kernel();
	printk(linux_banner);
...
void audioctl_ledtimer(unsigned long x)
{
    if (actl_ledtimerdisable==0) {
            if (actl_leddata[0]&actl_leddata[1]&actl_leddata[2]&actl_leddata[3]&actl_leddata[4]&actl_leddata[5]&actl_leddata[6]&actl_leddata[7]&LED_WHITE) {
                m8260_port_set(3,30,PORT_SET_PAR|PORT_SET_SOR);
                led_pwm_state=1;
            } else {
                m8260_port_set(3,30,PORT_CLEAR_PAR);
                led_pwm_state=0;
            }
...
void audioctl_set_leds(unsigned long arg)
{
    int green_port, red_port, green_pin, red_pin;

    if (sys_mdp.mdp_hwfeatures & MDP_HWFEATURE_LED_SWAP) {
        /* swapped units with red under play/pause button */
        red_port   = LED_GREEN_PORT;
        green_port = LED_RED_PORT;
        red_pin    = LED_GREEN_PIN;
        green_pin  = LED_RED_PIN;
    } else {
        /* unswapped units with green under mute button */
        red_port   = LED_RED_PORT;
        green_port = LED_GREEN_PORT;
        red_pin    = LED_RED_PIN;
    ...
    if (WEMBLEY)
        m8260_port_set(LED_WHITE_PORT,LED_WHITE_PIN,(arg&LED_WHITE)?PORT_SET_DAT:PORT_CLEAR_DAT); /*active high*/
    else
        m8260_port_set(LED_WHITE_PORT,LED_WHITE_PIN,(arg&LED_WHITE)?PORT_CLEAR_DAT:PORT_SET_DAT); /*active low*/
    m8260_port_set(red_port, red_pin, (arg&LED_RED)?PORT_CLEAR_DAT:PORT_SET_DAT); /*active low*/
    m8260_port_set(LED_AMBER_PORT,LED_AMBER_PIN,(arg&LED_AMBER)?PORT_CLEAR_DAT:PORT_SET_DAT); /*active low*/
    m8260_port_set(green_port, green_pin,(arg&LED_GREEN_BUTTON)?PORT_CLEAR_DAT:PORT_SET_DAT); /*active low*/
}
#endif

#if defined(CONFIG_PPC_SONOS) && defined(CONFIG_SONOS_DIAGS)
#define ZPS5_DIAGS ZPS5
#else
#define ZPS5_DIAGS 0
#endif
