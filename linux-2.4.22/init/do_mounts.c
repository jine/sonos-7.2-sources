...
extern char nandopt[64];
extern char nandjffs[64];
...
#endif
#ifdef CONFIG_SONOS
    if (nandroot[0]) sys_symlink(nandroot,"/dev/nandroot");
    if (nandopt[0]) sys_symlink(nandopt,"/dev/nandopt");
    if (nandjffs[0]) sys_symlink(nandjffs,"/dev/nandjffs");
    if (root_device_name[0]=='\0') {
        printk("Setting root_device_name to /dev/mtdblock_autobadmap/2\n");
        strcpy(root_device_name,"/dev/mtdblock_autobadmap/2");
    }
#endif
