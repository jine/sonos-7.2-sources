...
#ifdef CONFIG_SH_RINCON_ZONEPLAYER
#define DO_DXSUFLO			1			// reqd for Wifi&Enet PCI bus contention
#define PCNET32_EMULATE_EEPROM		1
#define ZONEPLAYER_MII_PORT		5
#define ZONEPLAYER_MII_PORT_SPEED	100
#define ZONEPLAYER_MII_DUPLEX		1
#endif
...
char default_ether_addr[6] = {
	0x08,0x88,0x01,0x01,0x01,0x44
};
...
static int  pcnet32_mii_port_scan( struct net_device *dev );
static void pcnet32_mii_auto_negotiate_disable( struct net_device *dev, int port, int speed, int duplex );
static void pcnet32_register_dump( struct net_device *dev );
...
#ifdef PCNET32_EMULATE_EEPROM

/*
 * Used if there is no EEPROM configured with the AMD79C973
 * Sets chip to default values normally contained within an EEPROM
 */
...
static EPROM_DEFAULTS eeprom_defaults[] = {
  {'B',2,0x3102},    //BCR2 handles Miscellaneous Configuration       
  {'B',4,0x00C0},    //BCR4 handles LED0 Status 
  {'B',5,0x0094},    //BCR5 handles LED1 Status    
  {'B',6,0x1080},    //BCR6 handles LED2 Status     
  {'B',7,0x0081},    //BCR7 handles LED3 Status
  {'B',9,0x0000},    //BCR9 handles Full-Duplex Control
  {'B',18,0x9261},   //BCR18 handles Burst and Bus Control
  {'B',22,0x1818},   //BCR22 handles PCI Latency
  {'B',23,0x1022},   //BCR23 handles PCI Subsystem Vendor ID
  {'B',24,0x2000},   //BCR24 handles PCI Subsystem ID
  {'B',25,0x0017},   //BCR25 handles SRAM Size
  {'B',26,0x0008},   //BCR26 handles SRAM Boundary
  {'B',27,0x0000},   //BCR27 handles SRAM Interface Control
  {'B',32,0x0020},   //BCR32 handles PHY Contro and Status
  {'B',33,0x03C0},   //BCR33 handles PHY Address
  {'B',35,0x1022},   //BCR35 handles PCI Vendor ID
  {'B',36,0xFE01},   //BCR36 handles PCI Power Management Capabilities Alias 
  {'B',37,0x0114},   //BCR37 handles PCI DATA Register Zero Alias
  {'B',38,0x010F},   //BCR38 handles PCI DATA Register One Alias
  {'B',39,0x010F},   //BCR39 handles PCI DATA Register Two Alias
  {'B',40,0x010F},   //BCR40 handles PCI DATA Register Three Alias
  {'B',41,0x0114},   //BCR41 handles PCI DATA Register Four Alias
  {'B',42,0x010F},   //BCR42 handles PCI DATA Register Five Alias
  {'B',43,0x010F},   //BCR43 handles PCI DATA Register Six Alias
  {'B',44,0x010F},   //BCR44 handles PCI DATA Register Seven Alias
  {'B',48,0x0000},   //BCR48 handles Node IP Address [15:0]
  {'B',49,0x0000},   //BCR49 handles Node IP Address [31:16]
  {'B',50,0x0000},   //BCR50 handles Management Station IEEE Address [15:0]
  {'B',51,0x0000},   //BCR51 handles Management Station IEEE Address [31:16]
  {'B',52,0x0000},   //BCR52 handles Management Station IEEE Address [47:32]
  {'B',53,0x0000},   //BCR53 handles Management Station IP Address [15:0]
  {'B',54,0x0000},   //BCR54 handles Management Station IP Address [31:16]
  {'B',55,0x0000},   //BCR55 handles SMIU Slave Address
  {'C',116,0x0058},  //CSR116 handles OnNow Miscellaneous
  {'Z',0,0}          //Z signifies the end of the table
};
  
static void	pcnet32_emulate_eeprom( unsigned long ioaddr,char *mac_addr )
{
  EPROM_DEFAULTS *ep;
  unsigned short val, n, csum, temp;

  ///////////////////////////////////////////////////////////////////
  for( ep = eeprom_defaults; ep->regtype != 'Z'; ep++ ) {
    switch( ep->regtype ){			//into the specific
      case 'B': 
		pcnet32_wio_write_bcr( ioaddr,ep->regnum,ep->regval );
                break;                 		//and
      case 'C': 
		pcnet32_wio_write_csr( ioaddr,ep->regnum,ep->regval );
      default:  
		break;                		//the table
      }
  }
  ...
  ///////////////////////////////////////////////////////////////////
  /* set MAC addr - PROM[0:5] */
  ///////////////////////////////////////////////////////////////////

    val = (mac_addr[1]<<8) | mac_addr[0];  	// are set by values from the
    outw( val,ioaddr );				// 0x00  write in IEEE address 
    pcnet32_wio_write_csr( ioaddr,12,val );	// write csr 12 with part MAC addr
						// from the MAC address that is set
    val = (mac_addr[3]<<8)|mac_addr[2];
    outw( val,ioaddr+2 );			// 0x01  write in IEEE address
    pcnet32_wio_write_csr( ioaddr,13,val );	// write csr 13 with part MAC addr

    val = (mac_addr[5]<<8) | mac_addr[4];
    outw( val,ioaddr+4 );			// 0x02  write in IEEE address
    pcnet32_wio_write_csr( ioaddr,14,val );	// write csr 14 with part MAC addr

  /////////////////////////////////////////////////////////////////  
  /* last  10 bytes of address PROM locations  */
  /////////////////////////////////////////////////////////////////  

    outw( 0x0058,ioaddr+6 );			// 0x03  CSR116  OnNow Power Mode
    outw( 0x1100,ioaddr+8 );			// 0x04  Hardware ID  Fixed value for MSB
    outw( 0x2222,ioaddr+10 );			// 0x05  User Progammable Free Space 
						// skip  location 0x06  which is CheckSum
    outw( 0x5757,ioaddr+14 );			// 0x07  Driver Key  Fixed value by
						// AMD device drivers
  
    csum = 0;
    for( n = 0; n < 12; n++ ) {  		// sum up the values
      if( n%2 == 0 ){				//  from 00h-0Bh
        temp = inw( ioaddr+n );
        csum += (temp&0xFF)+((temp&0xFF00)>>8);
      }
    }
    temp = inw( ioaddr+14 );   			// sum up the values
    csum += (temp&0xFF) + ((temp&0xFF00)>>8);	// from 0Eh-0FH
						// /resulting sum is the checksum value
    outw( csum,ioaddr+12);			// 0x06 checksum

    pcnet32_wio_write_bcr( ioaddr,2,0x2000 );	// BCR2 so BCR18[4:3] enabled
    pcnet32_wio_write_bcr( ioaddr,18,0x9271 );	// BCR18[4:3] = 0x10 -> external PHY
}
#endif
...
pcnet32_probe1(unsigned long ioaddr, unsigned char irq_line, int shared, int card_idx, struct pci_dev *pdev)
{
	...
	fdx = 1; mii = 1;
#ifdef CONFIG_SH_RINCON_ZONEPLAYER
	// when Wifi and AMD @ 100baseT are simultaneous - need this set
	// otherwise un-needed
	fset = 1;
#endif
	break;
	...
	val = a->read_csr(ioaddr, 80);
	val &= ~0x0F00;
	val |= 0x0C00;		/* set tx start pt full pkt, tx bus req earliest */
	a->write_csr(ioaddr, 80, val);
#ifdef DO_DXSUFLO
	dxsuflo = 1;
#endif
#ifndef CONFIG_SH_RINCON_ZONEPLAYER
	ltint = 1;
#endif
    }
    ...
    /* if were all zeros - then substitute an arbitrary addr */
    if( memcmp( dev->dev_addr,"\000\000\000\000\000\000",6 ) == 0 ) {
	memcpy( dev->dev_addr,default_ether_addr,6 );		/* load the default for now */
    }

    /* if the ethernet address is not valid, force to 00:00:00:00:00:00 */
    if( !is_valid_ether_addr(dev->dev_addr) ) {
	printk(" \nWarning: Ethernet address is either all zero or multicast - setting to zeros\n" );
	for (i = 0; i < 6; i++)
	    dev->dev_addr[i]=0;
    }
...
pcnet32_open(struct net_device *dev)
{
...
#ifdef PCNET32_EMULATE_EEPROM
    /* Setup pcnet32 if no EEPROM present */
    pcnet32_emulate_eeprom( ioaddr,dev->dev_addr );
#endif

#ifdef CONFIG_SH_RINCON_ZONEPLAYER

    /* set ZonePlayer MII port parameters */
    pcnet32_mii_auto_negotiate_disable( dev,ZONEPLAYER_MII_PORT,ZONEPLAYER_MII_PORT_SPEED,ZONEPLAYER_MII_DUPLEX );

    /* Scan for an mii phy_id so that we can query the link state */
    pcnet32_mii_port_scan( dev );
#endif
	...
	/* get phy control - bcr32 */
	val = lp->a.read_bcr (ioaddr, 32) & ~0x20;	/* reset XPHYANE - auto nego */
	val |= 0x80;					/* set DANAS */
	...
	val |= 0x800;		/* turn on auto poll enable */
    	lp->a.write_bcr( ioaddr,32,val );
    }
...
#ifdef CONFIG_SH_RINCON_ZONEPLAYER
// pcnet32_register_dump( dev );
	printk( KERN_INFO "MII CTL: %x MII STAT: %x\n",
		mdio_read( dev,5,0 ), mdio_read( dev,5,1 ) );
	i = 32;
	printk( KERN_INFO "BCR:%d:%x\n", i, lp->a.read_bcr (ioaddr, i));
	i++;
	printk( KERN_INFO "BCR:%d:%x\n", i, lp->a.read_bcr (ioaddr, i));
    
	printk( KERN_INFO "ANR:1:%x ANR:2:%x ANR:3:%x ANR:4:%x\n",
	mdio_read( dev,1,0x01 ), mdio_read( dev,2,0x01 ), mdio_read( dev,3,0x01 ), mdio_read( dev,4,0x01 ));
#endif
...
pcnet32_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    if (pcnet32_debug > 3) {
	printk(KERN_DEBUG "%s: pcnet32_start_xmit() called, csr0 %4.4x. addr:0x%x len: 0x%x\n",
	       dev->name, lp->a.read_csr (ioaddr, 0),(int)skb->data,skb->len);
    }
...
pcnet32_interrupt(int irq, void *dev_id, struct pt_regs * regs)
{
	...
	if (pcnet32_debug > 5)
	    printk(KERN_DEBUG "%s: interrupt  csr0=%#2.2x new csr0=%#2.2x.\n",
	...
	if (csr0 & 0x0200) {		/* Tx-done interrupt */
		...
		lp->tx_ring[entry].base = 0;
    		...
    		if (pcnet32_debug > 4)
			printk(KERN_DEBUG "%s: Tx Stat=%#4.4x:%#4.4x\n", dev->name,status,le32_to_cpu(lp->tx_ring[entry].misc));
    ...
    if (pcnet32_debug > 4)
	printk(KERN_DEBUG "%s: exiting interrupt, csr0=%#4.4x. bcr33=%#4.4x.\n",
	       dev->name, lp->a.read_csr (ioaddr, 0),
	       lp->a.read_bcr (ioaddr, 33));
...
pcnet32_rx(struct net_device *dev)
{
	...
	if (status != 0x03) {			/* There was an error. */
		...
		if (pkt_len > rx_copybreak) {
			...
			newskb->dev = dev;
                        lp->rx_dma_addr[entry] = pci_map_single(lp->pci_dev, newskb->tail, PKT_BUF_SZ, PCI_DMA_FROMDEVICE);
		    ...
		    skb = dev_alloc_skb(pkt_len+2);
                    pci_dma_sync_single(lp->pci_dev, lp->rx_dma_addr[entry],
...
{
    int i,stat;
    int val, sav_val = 0, sav_indx = 0;
    unsigned long ioaddr = dev->base_addr;
    struct pcnet32_private *lp = dev->priv;

    static int last_indx = -1;

    /* force external PHY mode */
    val = lp->a.read_bcr (ioaddr, 2);
    val |= 0x2000;
    lp->a.write_bcr (ioaddr, 2, val);

    val = lp->a.read_bcr (ioaddr, 18);
    val &= ~0x14;		/* turn off PHYSEL bits 4,3 */
    val |= 0x10;		/* set bits 4,3 = 10 */
    lp->a.write_bcr (ioaddr, 18, val);

    for( i = 1; i < PCNET32_MAX_PHY_ADDRS; i++ ) {
	...
	val = 0;
	stat = mdio_read( dev,i,MII_CTL_REG );
	if( stat&MII_CTL_SPEED_100M )
	    val |= 2;
	if( stat&MII_CTL_FD )
	    val |= 1;

	/* remember fastest, full duplex index */
	if( val > sav_val ) {
	    sav_val = val;
	    sav_indx = i;
	}
	...
	lp->mii_if.phy_id = sav_indx;
	lp->a.write_bcr (ioaddr, 33, ((sav_indx&0x1f)<<5) );

	if( sav_indx != last_indx ) {
	    printk( KERN_INFO "MII_PORT_SCAN: new link: %d\n",sav_indx );
	    last_indx = sav_indx;
	}
...
static void pcnet32_mii_auto_negotiate_disable( struct net_device *dev, int port, int speed, int duplex )
{
    ...
    val = speed ? MII_CTL_SPEED_100M : 0;
    val |= duplex ? MII_CTL_FD : 0;
    mdio_write( dev,port,MII_CTL_REG,MII_CTL_AN_DISABLE|val );
}
...
static void pcnet32_register_dump( struct net_device *dev )
{
    ...
    for( i = 0; i < 126; i += 4 )
	printk( KERN_INFO "CSR:%d:%x CSR:%d:%x CSR:%d:%x CSR:%d:%x\n",
	i, lp->a.read_csr (ioaddr, i), i+1, lp->a.read_csr (ioaddr, i+1), i+2, lp->a.read_csr (ioaddr, i+2), i+3, lp->a.read_csr (ioaddr, i+3));

    for( i = 0; i < 48; i += 4 )
	printk( KERN_INFO "BCR:%d:%x BCR:%d:%x BCR:%d:%x BCR:%d:%x\n",
	i, lp->a.read_bcr (ioaddr, i), i+1, lp->a.read_bcr (ioaddr, i+1), i+2, lp->a.read_bcr (ioaddr, i+2), i+3, lp->a.read_bcr (ioaddr, i+3));
    
    printk( KERN_INFO "ANR:1:%x ANR:2:%x ANR:3:%x ANR:4:%x\n",
	mdio_read( dev,1,MII_STAT_REG ), mdio_read( dev,2,MII_STAT_REG ), mdio_read( dev,3,MII_STAT_REG ), mdio_read( dev,4,MII_STAT_REG ));
}
