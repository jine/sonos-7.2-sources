 ...
 * This code is GPL
 *
 * $Id: mtdpart.c,v 1.8.24.2 2006/03/10 22:30:27 holmgren Exp $
 *
...
static int part_snap(struct mtd_info *mtd)
{
	...
	if (part->snap)
		return -EIO;
	sn=kmalloc(sizeof(u_char *)*(mtd->size/mtd->erasesize),GFP_KERNEL);
	if (!sn)
		return -ENOMEM;
	j=0;
	for (x=0;x<mtd->size;x+=mtd->erasesize) sn[j++]=0;
	j=0;
	for (x=0;x<mtd->size;x+=mtd->erasesize) {
		sn[j]=kmalloc(mtd->erasesize,GFP_KERNEL);
		if (!sn[j]) goto out_nomem;
		r=part_read(mtd,x,mtd->erasesize,&rl,sn[j]);
		if (r) goto out_fail;
		if (rl!=mtd->erasesize) { r=-EIO; goto out_fail; }
		j++;
	}
	part->snap=sn;
	return 0;
...
static void part_unsnap(struct mtd_info *mtd)
{
	struct mtd_part *part=PART(mtd);
	int x;
	if (part->snap) {
		for (x=0;x<(mtd->size/mtd->erasesize);x++) {
			kfree(part->snap[x]);
		}
		kfree(part->snap);
		part->snap=0;
...
static void part_setgeometry(struct mtd_info *mtd,struct partition_geometry *geo)
{
	struct mtd_part *part=PART(mtd);
	mtd->size=geo->length;
	part->offset=geo->start;
	printk("part_setgeometry:  mtd %p size %u offset %u\n",mtd,mtd->size,part->offset);
}
			...
			size_t *retlen, u_char *buf)
{
	...
	if (part->snap) {
		...
		while (p<(from+len)) {
			...
			sptr=part->snap[pd];
			sptr+=pr;
			cnt=mtd->erasesize-(pr);
			if (from+len<p+cnt) cnt=from+len-p;
			for (x=0;x<cnt;x++) *(buf++)=*(sptr++);
			p+=cnt;
		}
		*retlen=len;
		return 0;
		       ...
		       int nbparts)
{
	...
	for (i = 0; i < nbparts; i++) {
		...
		slave->mtd.snap = part_snap;
		slave->mtd.unsnap = part_unsnap;
		slave->mtd.setgeometry = part_setgeometry;
		slave->master = master;
