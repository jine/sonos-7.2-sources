 ...
 *
 * $Id: nand_base.c,v 1.9 2006/10/24 22:48:07 tober Exp $
 *
...
static int nand_read_special (struct mtd_info *mtd, struct mtd_special_info *rsi, void *b)
{
	struct nand_chip *this=mtd->priv;
	int x;
	nand_get_chip(this,mtd,FL_READING);
	...
	for (x=0;x<rsi->preop_cmdlen;x++) {
		this->write_byte(mtd,rsi->preop_cmd[x]);
	}
	this->hwcontrol(mtd, NAND_CTL_CLRCLE);
	this->hwcontrol(mtd, NAND_CTL_SETCLE);
	...
	this->hwcontrol(mtd, NAND_CTL_CLRCLE);
	this->hwcontrol(mtd, NAND_CTL_SETALE);
	for (x=0;x<rsi->addrlen;x++) {
		this->write_byte(mtd,rsi->addr[x]);
	}
	this->hwcontrol(mtd, NAND_CTL_CLRALE);
	udelay(1);
	if (this->dev_ready) {
...
static int nand_write_special (struct mtd_info *mtd, struct mtd_special_info *rsi, void *b)
{
	struct nand_chip *this=mtd->priv;
	int x;
	nand_get_chip(this,mtd,FL_WRITING);
	...
	this->hwcontrol(mtd, NAND_CTL_CLRALE);
        // Data Input
	for (x=0;x<rsi->datalen;x++) {
		this->write_byte(mtd, ((unsigned char *)b)[x]);
	}
        // Program Confirm Command
	this->hwcontrol(mtd, NAND_CTL_SETCLE);
	this->write_byte(mtd, 0x10);
//	this->hwcontrol(mtd, NAND_CTL_CLRCLE);
        // Delay
	udelay(1);
	if (this->dev_ready) {
        ...
        // We don't check the Read Status Register here.
        // Error checking is presumed to be done at the user level
...
int nand_scan (struct mtd_info *mtd, int maxchips)
{
	...
	mtd->devid = (nand_maf_id<<8)|nand_dev_id;
	...
	*/
	...
	mtd->read_special = nand_read_special;
	mtd->write_special = nand_write_special;
