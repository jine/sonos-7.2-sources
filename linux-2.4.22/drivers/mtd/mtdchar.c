/*
 * $Id: mtdchar.c,v 1.7 2006/10/24 22:48:07 tober Exp $
 *
		     ...
		     u_int cmd, u_long arg)
{
	...
	case MEMSETGEOMETRY:
	{
		struct partition_geometry geo;
		if (mtd->setgeometry) {
			if ((copy_from_user(&geo,(void *)arg,sizeof(geo)))) {
				return -EFAULT;
			}
			mtd->setgeometry(mtd,&geo);
			return 0;
		}
		return -EFAULT;
	}
	...
	case MEMREADSPECIAL:
	{
		...
		ret=verify_area(VERIFY_WRITE,(char *)rsi.databuf,rsi.datalen);
		if (ret) {
			goto readscrub;
		}
...
readscrub:
	...
	case MEMWRITESPECIAL:
	{
		struct mtd_special_info rsi;
		void *b;
		if (copy_from_user(&rsi, (void *)arg, sizeof(rsi)))
			return -EFAULT;
		if (!mtd->write_special)
			return -EOPNOTSUPP;
		b=kmalloc(rsi.datalen,GFP_KERNEL);
		if (!b) return -ENOMEM;
		if (copy_from_user(b, (void *)(rsi.databuf), rsi.datalen)) {
			ret = -EFAULT;
			goto writescrub;
		}
		if ((rsi.preop_cmdlen>8)||(rsi.addrlen>8)) {
		...
		ret=mtd->write_special(mtd,&rsi,b);
		if (ret) {
			goto writescrub;
		}
		ret=0;
