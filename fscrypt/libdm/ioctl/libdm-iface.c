...
static unsigned _dm_version_minor = DM_VERSION_MINOR;
...
static int _uname(void)
{
	...
	if (uname(&_uts)) {
		syslog(LOG_INFO, "uname failed: %s", strerror(errno));
		return 0;
	}
	...
	if (parts < 1 || (_kernel_major < 3 && parts < 3)) {
		syslog(LOG_INFO, "Could not determine kernel version used.");
		return 0;
	}
			    ...
			    uint32_t *number)
{
	...
	if (number) {
		syslog(LOG_INFO, "%s: No entry for %s found", file, name);
		return 0;
	}
...
static int _open_control(void)
{
	...
	if (!_create_dm_bitset()) {
		syslog(LOG_INFO, "Failed to set up list of device-mapper major numbers");
		return 0;
	}
...
bad:
	syslog(LOG_INFO, "Failure to communicate with kernel device-mapper driver.");
	if (!geteuid())
		syslog(LOG_INFO, "Check that device-mapper is available in the kernel.");
	return 0;
#else
	return 1;
#endif
...
int dm_task_get_driver_version(struct dm_task *dmt, char *version, size_t size)
{
	...
	if (version &&
		...
		syslog(LOG_INFO, "Buffer for version is to short.");
		if (size > 0)
			version[0] = '\0';
		return 0;
	}
...
static int _check_version(char *version, size_t size, int log_suppress)
{
	...
	if (!(task = dm_task_create(DM_DEVICE_VERSION))) {
		syslog(LOG_INFO, "Failed to get device-mapper version");
		version[0] = '\0';
		return 0;
	}
...
int dm_check_version(void)
{
	...
	syslog(LOG_INFO, "device-mapper ioctl protocol version %u failed. "
		    "Trying protocol version 1.", _dm_version);
	_dm_version = 1;
	if (_check_version(dmversion, sizeof(dmversion), 0)) {
		syslog(LOG_INFO, "Using device-mapper ioctl protocol version 1");
		return 1;
	}
	...
	syslog(LOG_INFO, "Incompatible libdevmapper %s%s and kernel driver %s",
...
uint32_t dm_task_get_read_ahead(const struct dm_task *dmt, uint32_t *read_ahead)
{
	...
	*read_ahead = 0;
	...
	if (*dmt->dmi.v4->name)
		...
		syslog(LOG_INFO, "Get read ahead request failed: device name unrecorded.");
		return 0;
	}
...
int dm_task_set_add_node(struct dm_task *dmt, dm_add_node_t add_node)
{
	...
	case DM_ADD_NODE_ON_CREATE:
		...
		return 1;
	default:
		syslog(LOG_INFO, "Unknown add node parameter");
		return 0;
	}
}
			     ...
			     const char *params)
{
	...
	if (strlen(type) >= DM_MAX_TYPE_NAME) {
		syslog(LOG_INFO, "Target type name %s is too long.", type);
		return NULL;
	}

	if (!(t = dm_zalloc(sizeof(*t)))) {
		syslog(LOG_INFO, "create_target: malloc(%" PRIsize_t ") failed",
			  sizeof(*t));
		return NULL;
	}

	if (!(t->params = dm_strdup(params))) {
		syslog(LOG_INFO, "create_target: strdup(params) failed");
		goto bad;
	}

	if (!(t->type = dm_strdup(type))) {
		syslog(LOG_INFO, "create_target: strdup(type) failed");
		goto bad;
	}
...
static char *_add_target(struct target *t, char *out, char *end)
{
	...
	if (strlen(t->type) >= sizeof(sp.target_type)) {
		syslog(LOG_INFO, "Target type name %s is too long.", t->type);
		return NULL;
	}
	...
	out += sp_size;
	pt = t->params;
	//syslog(LOG_INFO, "_add_target params %s", t->params);
	...
	if ((out >= end) || (out + len + 1) >= end) {
		syslog(LOG_INFO, "Ran out of memory building ioctl parameter");
		return NULL;
	}
...
static struct dm_ioctl *_flatten(struct dm_task *dmt, unsigned repeat_count)
{
	...
	if (count && (dmt->sector || dmt->message)) {
		syslog(LOG_INFO, "targets and message are incompatible");
		return NULL;
	}

	if (count && dmt->newname) {
		syslog(LOG_INFO, "targets and rename are incompatible");
		return NULL;
	}

	if (count && dmt->geometry) {
		syslog(LOG_INFO, "targets and geometry are incompatible");
		return NULL;
	}

	if (dmt->newname && (dmt->sector || dmt->message)) {
		syslog(LOG_INFO, "message and rename are incompatible");
		return NULL;
	}

	if (dmt->newname && dmt->geometry) {
		syslog(LOG_INFO, "geometry and rename are incompatible");
		return NULL;
	}

	if (dmt->geometry && (dmt->sector || dmt->message)) {
		syslog(LOG_INFO, "geometry and message are incompatible");
		return NULL;
	}

	if (dmt->sector && !dmt->message) {
		syslog(LOG_INFO, "message is required with sector");
		return NULL;
	}
	...
	if (dmt->minor >= 0) {
		if (dmt->major <= 0) {
			syslog(LOG_INFO, "Missing major number for persistent device.");
			goto bad;
		}
	...
	if (_dm_version_minor < 3 && !DEV_NAME(dmt) && !DEV_UUID(dmt) && dmi->dev) {
		if (!_lookup_dev_name(dmi->dev, dmi->name, sizeof(dmi->name))) {
			syslog(LOG_INFO, "Unable to find name for device (%" PRIu32
				  ":%" PRIu32 ")", dmt->major, dmt->minor);
			goto bad;
		}
		syslog(LOG_INFO, "device (%" PRIu32 ":%" PRIu32 ") is %s "
	...
	if (DEV_NAME(dmt) && (dmt->type != DM_DEVICE_RESUME || dmt->minor < 0 ||
			      dmt->major < 0)) {
		strncpy(dmi->name, DEV_NAME(dmt), sizeof(dmi->name) - 1);
		dmi->name[sizeof(dmi->name) - 1] = '\0';
	}

	if (DEV_UUID(dmt)) {
		strncpy(dmi->uuid, DEV_UUID(dmt), sizeof(dmi->uuid) - 1);
		dmi->uuid[sizeof(dmi->uuid) - 1] = '\0';
		syslog(LOG_INFO, "_flatten DEV_UUID %s", dmi->uuid);
	}
	...
	if (dmt->secure_data) {
		if (_dm_version_minor < 20)
			syslog(LOG_INFO, "Secure data flag unsupported by kernel. "
				    "Buffers will not be wiped after use.");
		dmi->flags |= DM_SECURE_DATA_FLAG;
	}
	if (dmt->new_uuid) {
		if (_dm_version_minor < 19) {
			syslog(LOG_INFO, "WARNING: Setting UUID unsupported by "
...
static int _create_and_load_v4(struct dm_task *dmt)
{
	...
	if (!dm_task_run(dmt))
		syslog(LOG_ERR, "Failed to revert device creation.");
...
static int _reload_with_suppression_v4(struct dm_task *dmt)
{
	...
	if (!(task = dm_task_create(DM_DEVICE_TABLE))) {
		syslog(LOG_ERR, "Failed to create device-mapper task struct");
		return 0;
	}
					...
					dm_string_mangling_t mode)
{
	...
	if ((r = unmangle_string(str, str_name, strlen(str), buf, buf_size, mode)) < 0) {
		syslog(LOG_ERR, "_do_dm_ioctl_unmangle_string: failed to "
...
static int _dm_ioctl_unmangle_uuids(struct dm_ioctl *dmi)
{
				     ...
				     int *retryable)
{
	...
	if (!dmi) {
		syslog(LOG_ERR, "Couldn't create ioctl argument.");
		return NULL;
	}
	...
	if (ioctl(_control_fd, command, dmi) < 0 &&
		...
		if (errno == ENXIO && ((dmt->type == DM_DEVICE_INFO) ||
			...
			if (_log_suppress)
				syslog(LOG_INFO, "device-mapper: %s ioctl "
					    "failed: %s",
				    	    _cmd_data_v4[dmt->type].name,
					    strerror(errno));
			else
				syslog(LOG_INFO, "device-mapper: %s ioctl on %s "
					  "failed: %s",
					  _cmd_data_v4[dmt->type].name,
...
int dm_task_run(struct dm_task *dmt)
{
	...
	if ((unsigned) dmt->type >=
		...
		syslog(LOG_INFO, "unknown device-mapper task %d",
			  dmt->type);
		return 0;
	}
	...
	if ((suspended_counter = dm_get_suspended_counter()) &&
		...
		syslog(LOG_INFO, "Performing unsafe table load while %d device(s) "
...
repeat_ioctl:
	...
	if (dmi->flags & DM_BUFFER_FULL_FLAG) {
		...
		case DM_DEVICE_WAITEVENT:
			...
			goto repeat_ioctl;
		default:
			syslog(LOG_INFO, "WARNING: libdevmapper buffer too small for data");
		}
	}
