...
char *dm_strdup_aux(const char *str, const char *file, int line)
{
	...
	if (!str) {
		syslog(LOG_INFO, "dm_strdup called with NULL pointer");
		return NULL;
	}
...
void *dm_malloc_aux_debug(size_t s, const char *file, int line)
{
	...
	if (s > 50000000) {
		syslog(LOG_INFO, "Huge memory allocation (size %" PRIsize_t
			  ") rejected - metadata corruption?", s);
		return 0;
	}

	if (!(nb = malloc(tsize))) {
		syslog(LOG_INFO, "couldn't allocate any memory, size = %" PRIsize_t,
			  s);
		return 0;
	}
...
int dm_dump_memory_debug(void)
{
	...
	if (_head)
		syslog(LOG_INFO, "You have a memory leak:");
	...
	if (_head)
		syslog(LOG_INFO, "%ld bytes leaked in total", tot);
