...
static int device_ready(const char *device)
{
	...
	if (devfd < 0) {
		syslog(LOG_ERR, "Device %s doesn't exist or access denied.", device);
		return -EINVAL;
	}
	if (fstat(devfd, &st) < 0) {
		r = -EINVAL;
...
int device_open(struct device *device, int flags)
{
	...
	if (devfd < 0 && errno == EINVAL) {
		syslog(LOG_ERR, "Trying to open device %s without direct-io.",
			device_path(device));
...
void device_free(struct device *device)
{
	...
	if (device->loop_fd != -1) {
		syslog(LOG_ERR, "Closed loop %s (%s).", device->path, device->file_path);
			    ...
			    unsigned long default_alignment)
{
	...
	*alignment_offset = 0;
	...
	if (ioctl(fd, BLKIOMIN, &min_io_size) == -1) {
		syslog(LOG_ERR, "Topology info for %s not supported, using default offset %lu bytes.",
			...
			uint32_t *flags)
{
	...
	if (r < 0) {
		if (r == -EBUSY)
			syslog(LOG_ERR, "Cannot use device %s which is in use "
				      "(already mapped or mounted).",
				      device->path);
		else
			syslog(LOG_ERR, "Cannot get info about device %s.",
				device->path);
		return r;
	}

	if (device_offset >= real_size) {
		syslog(LOG_ERR, "Requested offset is beyond real size of device %s.",
			device->path);
		return -EINVAL;
	}
	...
	if (size && !*size) {
		*size = real_size;
		if (!*size) {
			syslog(LOG_ERR, "Device %s has zero size.", device->path);
			return -ENOTBLK;
		}
		*size -= device_offset;
	}
	...
	if (size && ((real_size - device_offset) < *size)) {
		syslog(LOG_ERR, "Device %s: offset = %" PRIu64 " requested size = %" PRIu64
			", backing device size = %" PRIu64,
			device->path, device_offset, *size, real_size);
		syslog(LOG_ERR, "Device %s is too small.", device->path);
		return -EINVAL;
	}
	...
	if (size)
		syslog(LOG_INFO, "Calculated device size is %" PRIu64" sectors (%s), offset %" PRIu64 ".",
