...
int br_set_uplink_mode(struct bridge *br, int uplink_mode)
{
	if (br_device_ioctl(br, BRCTL_SET_UPLINK_MODE, uplink_mode,
			    0, 0) < 0)
		return errno;
...
int br_set_forwarding_state(struct bridge *br, int forwarding_state)
{
	if (br_device_ioctl(br, BRCTL_SET_BRIDGE_FORWARDING_STATE, forwarding_state,
			    0, 0) < 0)
		return errno;
...
int br_read_mcast_fdb(struct bridge *br, unsigned char *buf, int len,
                      int offset)
{
    int numread;

    if ((numread = br_device_ioctl(br, BRCTL_MCAST_GET_FDB_ENTRIES,
                                   (unsigned long)buf, len, offset)) < 0)
                return errno;

    return numread;
}

int br_add_p2p_tunnel(struct bridge *br, int ifindex, unsigned char *daddr,
                      int path_cost)
{
        if (br_device_ioctl(br, BRCTL_ADD_P2P_TUNNEL, 
                            ifindex, (unsigned long)daddr, path_cost) < 0)
                return errno;

        return 0;
}

int br_set_p2p_tunnel_path_cost(struct bridge *br, int ifindex, 
                                unsigned char *daddr, int path_cost)
{
        if (br_device_ioctl(br, BRCTL_SET_P2P_TUNNEL_PATH_COST, 
                            ifindex, (unsigned long)daddr, path_cost) < 0)
                return errno;
...
int br_del_p2p_tunnel(struct bridge *br, int ifindex, unsigned char *daddr)
{
        if (br_device_ioctl(br, BRCTL_DEL_P2P_TUNNEL, 
                            ifindex, (unsigned long)daddr, 0) < 0)
                return errno;

        return 0;
}

int br_get_p2p_tunnel_states(struct bridge *br, int ifindex, 
                             unsigned char *state_data,
                             unsigned int *state_data_len)
{
        if (br_device_ioctl(br, BRCTL_GET_P2P_TUNNEL_STATES, 
                            ifindex,    
                            (unsigned long)state_data, 
                            (unsigned long)state_data_len) < 0)
                return errno;

        return 0;
}

int br_set_mac(struct bridge *br, unsigned char *daddr)
{
        if (br_device_ioctl(br, BRCTL_SET_STATIC_MAC,
                            (unsigned long)daddr,
                            0,
                            0) < 0)
                return errno;
