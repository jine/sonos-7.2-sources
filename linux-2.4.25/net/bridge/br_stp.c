 ...
 *
 *	$Id: br_stp.c,v 1.1.1.1 2006/12/23 00:48:21 holmgren Exp $
 *
...
void br_log_state(struct net_bridge_port *p, char *state)
{
    char rgchMacBuf[64];
    if (p->is_p2p)
        sprintf(rgchMacBuf,
                "; dest MAC %02x:%02x:%02x:%02x:%02x:%02x",
                p->p2p_dest_addr[0], p->p2p_dest_addr[1], p->p2p_dest_addr[2],
                p->p2p_dest_addr[3], p->p2p_dest_addr[4], p->p2p_dest_addr[5]);
    else
        rgchMacBuf[0] = '\0';

    printk(KERN_DEBUG "%s: port %i(%s%s) entering %s state\n",
           p->br->dev.name, p->port_no, p->dev->name, rgchMacBuf, state);
}
...
{
	printk(KERN_DEBUG "%s: topology change detected", br->dev.name);
...
static void br_make_forwarding(struct net_bridge_port *p)
{
	if (p->state == BR_STATE_BLOCKING) {
		if (p->br->stp_enabled) {
			br_log_state(p, "listening");
			p->state = BR_STATE_LISTENING;
		} else {
			br_log_state(p, "learning");
			p->state = BR_STATE_LEARNING;
		}
...
void br_received_tcn_bpdu(struct net_bridge_port *p)
{
	...
	if (p->state != BR_STATE_DISABLED &&
	    br_is_designated_port(p)) {
		printk(KERN_DEBUG "%s: received tcn bpdu on port %i(%s)\n",
