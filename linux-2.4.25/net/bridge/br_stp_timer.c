 ...
 *
 *	$Id: br_stp_timer.c,v 1.1.1.1 2006/12/23 00:48:21 holmgren Exp $
 *
...
static void br_message_age_timer_expired(struct net_bridge_port *p)
{
	...
	br = p->br;
	printk(KERN_DEBUG "%s: ", br->dev.name);
	printk("neighbour ");
...
{
	printk(KERN_DEBUG "%s: retransmitting tcn bpdu\n", br->dev.name);
	br_transmit_tcn(br);
	br_timer_set(&br->tcn_timer, jiffies);
}

/* called under bridge lock */
static void br_mcast_timer_expired(struct net_bridge *br)
{
	br_mcast_transmit_grouplist(br);
	br_timer_set(&br->mcast_timer, jiffies);
}
...
static void br_check_timers(struct net_bridge *br)
{
	...
	if (br_timer_has_expired(&br->tcn_timer, br->bridge_hello_time)) {
        ...
        if (br_timer_has_expired(&br->mcast_timer, br->mcast_advertise_time)) {
                br_timer_clear(&br->mcast_timer);
                br_mcast_timer_expired(br);
        }
