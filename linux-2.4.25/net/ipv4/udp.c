 ...
 *
 * Version:	$Id: udp.c,v 1.1.1.1 2006/12/23 00:48:22 holmgren Exp $
 *
		...
		int noblock, int flags, int *addr_len)
{
	...
	if (sin)
	{
		...
		if (sk->sonos_options & SO_SONOS_OPTION_SONOS_SOCKADDR) {
			 ...
			 */
			char *eth = (char *)skb->mac.ethernet;
			char *ip  = (char *)skb->nh.iph;
			...
			if ((ip > eth) && (ip - eth == sizeof(struct ethhdr))) {
				...
				memcpy(&p->mac[0], skb->mac.ethernet->h_source, ETH_ALEN);
			}
		}
#endif
  	}
