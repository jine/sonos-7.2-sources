/*
 * $Id: mtdchar.c,v 1.1.1.1 2007/03/19 16:40:08 holmgren Exp $
 *
...
static ssize_t mtd_read(struct file *file, char *buf, size_t count,loff_t *ppos)
{
	...
	while (count) {
		...
		ret = MTD_READ(mtd, *ppos, len, &retlen, xbuf);
		if (!ret) {
			*ppos += retlen;
			if (copy_to_user(buf, xbuf, retlen)) {
				return -EFAULT;
			}
			else
				total_retlen += retlen;
...
static ssize_t mtd_write(struct file *file, const char *buf, size_t count,loff_t *ppos)
{
	...
	while (count) {
		...
		if (copy_from_user(xbuf, buf, len)) {
			return -EFAULT;
		}
		
	        ret = (*(mtd->write))(mtd, *ppos, len, &retlen, xbuf);
		if (!ret) {
			*ppos += retlen;
			total_retlen += retlen;
			count -= retlen;
...
static int __init init_mtdchar(void)
{
	...
	if (!xbuf) {
		printk(KERN_NOTICE "init_mtdchar: kmalloc failed\n");
		return -ENOMEM;
	}
#ifdef CONFIG_DEVFS_FS
