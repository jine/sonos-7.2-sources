...
#ifdef	CONFIG_SONOS
// moving these defines to a header is beyond current scope. 
#define MII_STAT_REG            0x01
#define MII_STAT_LINK_UP        0x04
	stat = mii->mdio_read(dev, mii->phy_id, MII_STAT_REG);
        if(  stat != 0xffff && stat != 0x0000 && (stat&MII_STAT_LINK_UP) == 0 ) {
		// no-link is reported as 0 speed
		ecmd->duplex = ecmd->speed = 0;
	}
#endif	// CONFIG_SONOS
