...
#ifdef CONFIG_PERICOM
	/*
	 * The writes to the pci_command register that follow this for-loop
	 * result int a panic/die for some unknown reason. This for-loop
	 * prevents that crash. Having this as a hack/WAR temporarily until
	 * we root cause the issue.
	 */
	for (cmd = 0; cmd < 100; cmd ++) {
		printk("%s(%d) - pci local reset 0x%x\n", __func__,
			__LINE__, ar7240_reg_rd(AR7240_PCI_LCL_RESET));
		udelay(10);
	}
