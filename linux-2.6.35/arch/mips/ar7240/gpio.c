...
struct timer_list  doublepress_time_t;
#define frdbg printk
...
int32_t register_simple_config_callback (char *cbname, void *callback, void *arg1, void *arg2)
{
	...
	if(!SCcallback[0].name){
        ...
        SCcallback[0].name = (char*)kmalloc(strlen(cbname) + 1, GFP_KERNEL);
        ...
        /* SONOS(MartyC):  add one for terminating null */
        SCcallback[1].name = (char*)kmalloc(strlen(cbname) + 1, GFP_KERNEL);
...
static OS_TIMER_FUNC(wps_doublepress_button)						//jim
{
	printk(KERN_ERR "In MwInform,the paravalues is PushKeyEvent=WPSSHORT\n");
	double_flag = 0;	
	OS_CANCEL_TIMER(&doublepress_time_t);
}
...
#define UDELAY_COUNT 4000
        ...
        if (!delay) {
            ...
            printk(KERN_ERR "In MwInform,the paravalues is PushKeyEvent=WPSLONG\n");
            printk(KERN_ERR "In MwInform,the paravalues is PushKeyEvent=FactoryReset\n");
            local_irq_enable();
            return IRQ_HANDLED;
        } else {
        	 if (double_flag == 0)
        	 {
				double_flag=1;
				OS_INIT_TIMER(NULL, &doublepress_time_t, wps_doublepress_button, &doublepress_time_t);
        		OS_SET_TIMER(&doublepress_time_t, 2000);
        	 }
        	 else{
        	 	double_flag=0;
				printk(KERN_ERR "In MwInform,the paravalues is PushKeyEvent=WPSDOUBLE\n");
				OS_CANCEL_TIMER(&doublepress_time_t);
        	 } 
