...
typedef struct tdm83xx {
    ...
    /* TDM Configuration (SB, 0x00016000) */
	u32 tdmgir;
	u32 tdmrir;
	u32 tdmtir;
	u32 tdmrfp;
    ...
    /* TDM Data (AHB) (0x00016100) */
    u64 tdmrdreg;
    u64 tdmtdreg;
    u8  res7[0x70];

    /* TDM Clock (0x000016180) */
    u32 tdmclk_div_val_rx;
    u32 tdmclk_div_val_tx;
    u8  res8[0x78];
...
typedef struct tcd_t
{
    u32 saddr;      /* 0x00 Source Address */
    u16 smod:5,     /* 0x04 Source Modulo */
        ssize:3,    /*      Source Data Transfer Size */
        dmod:5,     /*      Destination Modulo */
        dsize:3;    /*      Destination Data Transfer Size */
    u16 soff;       /* 0x06 Signed Source Address Offset */

    /* Note: Assumes DMACR[EMLM] == 0 (Disable minor loop mapping) */
    u32 nbytes;     /* 0x08 Byte Transfer Count */
    u32 slast;      /* 0x0C Last Source Address Adjustment */
    u32 daddr;      /* 0x10 Destination address */

    /* Note: Assumes MSb == 0 (Disable channel-to-channel-linking on minor loop complete) */
    u16 citer;      /* 0x14 Enable channel-to-channel-linking on Minor Loop Complete */
    ...
    /* Note: Assumes MSb == 0 (Disable channel-to-channel-linking on minor loop complete) */
    u16 biter;      /* 0x1C Beginning Major Iteration Count */

    /* TCD Control and Status */
    u16 bwc:2,      /* 0x1E Bandwidth Control */
        linkch:6,   /*      Link Channel Number */
        done:1,     /*      Channel Done */
        active:1,   /*      Channel Active */
        e_link:1,   /*      Enable channel-to-channel-linking on Major Loop Complete */
        e_sg:1,     /*      Enable Scatter/Gather Processing */
        d_req:1,    /*      Disable Request */
        int_halt:1, /*      Enable an Interrupt When Counter is Half Complete */
        int_maj:1,  /*      Enable an Interrupt When Major Iteration Count Completes */
        start:1;    /*      Channel Start */
} tcd_st;
