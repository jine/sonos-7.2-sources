...
void fsl_elbc_custom_command(struct mtd_info *mtd, unsigned int cmd)
{
	struct nand_chip *chip = mtd->priv;
	struct fsl_elbc_mtd *priv = chip->priv;
	struct fsl_lbc_ctrl *ctrl = priv->ctrl;
	...
	out_be32(&lbc->fir, FIR_OP_CM0 << FIR_OP0_SHIFT);
	out_be32(&lbc->fcr, cmd << FCR_CMD0_SHIFT);
	fsl_elbc_run_command(mtd);
}
...
static u8 fsl_elbc_read_byte(struct mtd_info *mtd)
{
	...
	dev_err(priv->dev, "read_byte beyond end of buffer index %d read_bytes %d\n", elbc_fcm_ctrl->index, elbc_fcm_ctrl->read_bytes);
	return ERR_BYTE;
}
...
static int fsl_elbc_read_special(struct mtd_info *mtd, struct mtd_special_info *rsi, void *buf)
{
	...
	nand_get_device_exp(chip, mtd, FL_READING);
        ...
        if (!rsi->alternate_read && rsi->preop_cmdlen) {
		// This is read special for small page and SPANSION large page
		// NAND flashes.  For ONFI-compliant devices, the alternate_read
		// will be set with no preop_cmdlen.

		/* disable ECC */
		br = ecc = in_be32(&lbc->bank[priv->bank].br);
		br &= ~BR_DECC;
		out_be32(&lbc->bank[priv->bank].br, br);

		fsl_elbc_wait(mtd, chip);
		...
		*/
		if (0x01f1 == mtd->devid) {
			fsl_elbc_cmdfunc(mtd, NAND_CMD_RESET, 0, 0);
			fsl_elbc_wait(mtd, chip);
		}
		...
		if (rsi->preop_cmdlen) {
		    for (x=0;x<(rsi->preop_cmdlen);x++) {
				fsl_elbc_custom_command(mtd,rsi->preop_cmd[x]);
		    }
		}

		if ( (mtd->devid == 0x01f1) && (rsi->preop_cmdlen == 4) &&
		     (rsi->preop_cmd[0] == 0x29) && (rsi->preop_cmd[1] == 0x16) &&
		     (rsi->preop_cmd[2] == 0x02) && (rsi->preop_cmd[3] == 0x19) ) {
			// Read unique ID, large-page SPANSION, needs page 2
			fsl_elbc_cmdfunc(mtd,NAND_CMD_READ0,0,2);
		} else {
			fsl_elbc_cmdfunc(mtd,NAND_CMD_READ0,0,0);
		}
		fsl_elbc_read_buf(mtd, buf, (rsi->datalen>256)?256:rsi->datalen);
		fsl_elbc_cmdfunc(mtd,NAND_CMD_RESET,0,0);
		fsl_elbc_wait(mtd,chip);

		/* restore ECC */
		out_be32(&lbc->bank[priv->bank].br, ecc);
	} else {
		// This path supports the ONFI Read Unique ID command.
		...
		if (rsi->alternate_read) {
		    ...
		    */
		    ...
		    out_be32(&lbc->fir,
		         ...
		         (FIR_OP_RBW << FIR_OP2_SHIFT));
		    out_be32(&lbc->fcr,
			     (rsi->alternate_read << FCR_CMD0_SHIFT) );

		    out_be32(&lbc->fbcr, rsi->datalen);
		    elbc_fcm_ctrl->read_bytes = rsi->datalen;
		    elbc_fcm_ctrl->use_mdr = 1;
		    elbc_fcm_ctrl->mdr = rsi->addr[0];
		    fsl_elbc_run_command(mtd);
		} else {
		    set_addr(mtd, 0, 0, 0);
		    fsl_elbc_cmdfunc(mtd, NAND_CMD_READ0, 0, 0);
...
static int fsl_elbc_chip_init(struct fsl_elbc_mtd *priv)
{
	...
	/* set up nand options */
#ifdef CONFIG_SONOS
   /* we don't have the bad block table in flash */
					 ...
					 const struct of_device_id *match)
{
	...
	if (bank >= MAX_BANKS) {
		dev_err(fsl_lbc_ctrl_dev->dev, "address did not match any "
			"chip selects %i %i %x %x\n", bank, MAX_BANKS, in_be32(&lbc->bank[bank].br), res.start);
		return -ENODEV;
	}
	...
	priv->mtd.read_special = fsl_elbc_read_special;
	...
	if (virt_mtd == NULL)
		return -ENOMEM;
#else
	/* Virtual not enabled so the virtual MTD is the physical MTD */
	virt_mtd = &priv->mtd;
#endif

	// printk("Calling nand_scan_tail, MTD %p\n", &priv->mtd);
...
#ifdef CONFIG_MTD_NAND_SONOS_VNB_MAPPING
	/* This is done after nand_scan_tail so that we get the latest data */
