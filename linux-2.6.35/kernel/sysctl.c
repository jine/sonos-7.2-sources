...
#ifdef CONFIG_SONOS
/* Printing is controlled by sys_mdp.mdp_flags & MDP_FLAG_KERNEL_PRINTK_ENABLE
 * when this is set printk output is directed to the console uart.
 * printk output is always directed to the logger (dmesg)
 *
 * on limelight:
 * the sys_mdp struct is read by uboot and passed to the kernel at a 
 * well know address.
 *
 * on fenway/anvil/ameoba:
 * the sys_mdp struct is initialized to 0. Later in the boot the mdp is  
 * read into the sys_mdp struct. 
 * The default behavior is to suppress printing and not initialize the UART.
 * When uboot sees the MDP_FLAG_KERNEL_PRINTK_ENABLE flag it adds 
 * enable_printk=1 to the commandline.
 * When uboot sees the MDP_FLAG_CONSOLE_ENABLE flag it adds 
 * enable_8250=1 to the command line.
 * __setup processing updates sys_mdp.mdp_flags as needed
 * 
 * In both cases sys_mdp is a (possibly inconsistent) cache of the mdp in flash.
 */
...
static int __init enable_printk_setup(char *str)
{
	...
	// fenway: when uboot passes enable_printk it always passes enable_printk=1
	enable_printk = simple_strtoul(str, NULL, 1);
