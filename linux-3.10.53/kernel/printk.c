...
struct log {
	u64 ts_nsec;		/* timestamp in nanoseconds (or jiffies/HZ if USE_TIME is not defined) */
...
static size_t print_time(u64 ts, char *buf)
{
	...
	if (!printk_time)
		return 0;
	if (!buf)
		return snprintf(NULL, 0, "(%lu)", (unsigned long)ts);
	return sprintf(buf, "(%lu)", (unsigned long)ts);
#endif
}
