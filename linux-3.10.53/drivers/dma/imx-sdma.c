...
#ifdef CONFIG_SONOS
/*
 * The i.MX6 is not cache coherent between the ARM cores and internal
 * peripherals.  Therefore manual cache syncs are required to ensure
 * the SDMA and ARM core(s) are not using stale data.
 */
#define SDMA_CACHE_SYNC
/*
...
{
	struct sdma_engine *sdma = m->private;
#ifdef SDMA_CACHE_SYNC
	dma_sync_single_for_cpu(sdma->dev, sdma->channel_control_phys,
			(MAX_DMA_CHANNELS * sizeof(struct sdma_channel_control)) + sizeof(struct sdma_context_data), DMA_FROM_DEVICE);
#endif // SDMA_CACHE_SYNC
...
#ifdef SDMA_CACHE_SYNC
	dma_sync_single_for_device(sdma->dev, sdma->channel[0].bd_phys, sizeof(struct sdma_buffer_descriptor), DMA_TO_DEVICE);
	dma_sync_single_for_device(sdma->dev, buf_phys, size, DMA_TO_DEVICE);
#endif // SDMA_CACHE_SYNC
...
static void sdma_handle_channel_loop(struct sdma_channel *sdmac)
{
	...
	while (1) {
#ifdef SDMA_CACHE_SYNC
		dma_sync_single_for_cpu(sdmac->sdma->dev,
				sdmac->bd_phys + (sdmac->buf_tail * sizeof(struct sdma_buffer_descriptor)),
				sizeof(struct sdma_buffer_descriptor), DMA_FROM_DEVICE);
#endif // SDMA_CACHE_SYNC
...
#ifdef SDMA_CACHE_SYNC
		dma_sync_single_for_device(sdmac->sdma->dev,
			sdmac->bd_phys + (sdmac->buf_tail * sizeof(struct sdma_buffer_descriptor)),
			sizeof(struct sdma_buffer_descriptor), DMA_TO_DEVICE);
#endif // SDMA_CACHE_SYNC
...
static void mxc_sdma_handle_channel_normal(struct sdma_channel *sdmac)
{
	 ...
	 */
#ifdef SDMA_CACHE_SYNC
	dma_sync_single_for_cpu(sdmac->sdma->dev, sdmac->bd_phys, PAGE_SIZE, DMA_FROM_DEVICE);
#endif // SDMA_CACHE_SYNC
...
#ifdef SDMA_CACHE_SYNC
	dma_sync_single_for_device(sdma->dev, sdma->channel[0].bd_phys, sizeof(struct sdma_buffer_descriptor), DMA_TO_DEVICE);
	dma_sync_single_for_device(sdma->dev, sdma->context_phys, sizeof(struct sdma_context_data), DMA_TO_DEVICE);
#endif // SDMA_CACHE_SYNC
...
#ifdef SDMA_CACHE_SYNC
	dma_sync_single_for_device(sdma->dev, sdmac->bd_phys, PAGE_SIZE, DMA_TO_DEVICE);
	dma_sync_single_for_device(sdma->dev, (sdma->channel_control_phys + (channel * sizeof(struct sdma_channel_control))),
			sizeof(struct sdma_channel_control), DMA_TO_DEVICE);
#endif // SDMA_CACHE_SYNC
...
#ifdef SDMA_CACHE_SYNC
	dma_sync_single_for_cpu(sdmac->sdma->dev,
			sdmac->bd_phys + (index * sizeof(struct sdma_buffer_descriptor)),
			sizeof(struct sdma_buffer_descriptor), DMA_FROM_DEVICE);
#endif // SDMA_CACHE_SYNC
...
#ifdef SDMA_CACHE_SYNC
	dma_sync_single_for_device(sdma->dev, sdma->channel_control_phys,
			MAX_DMA_CHANNELS * sizeof(struct sdma_channel_control), DMA_TO_DEVICE);
#endif // SDMA_CACHE_SYNC
