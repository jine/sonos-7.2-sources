	...
	size_t *retlen, u_char *buf)
{
	...
	if ( !strncmp(cal_part,"mtdblock",8 ) ) {
		// device name - don't need to walk the list
		sscanf(&cal_part[8],"%d",&i);
		mtd = __mtd_next_device(i);
		printk(KERN_INFO "reading %d bytes from %s - %s [%d]\n",len,cal_part,mtd->name,i);
	}
	else {
		for ( i = 0; i < 10; i++ ) {
			mtd = __mtd_next_device(i);
	...
	// Need to use this interface to validate rootfs digest before mounting,
	// but want to leave things in their status quo ante state, so if this
	// was opened for rootfs, close the volume.
...
void get_rootfs_ubi_info(struct mtd_info * mtd, int *vol_id, int *ubi_num)
{
	...
	gluebi = container_of(mtd, struct gluebi_device, mtd);
	if ( gluebi == NULL ) {
		printk("No gluebi of %s\n", mtd->name);
		return;
	}
	*vol_id = gluebi->vol_id;
	*ubi_num = gluebi->ubi_num;
}
#endif
