...
int get_imx6_fuse(const char* name, uint32_t* pValue)
{
	/* only one supported for now */
	return strcmp(name, SONOS_FUSE_UNLOCK_CTR_NAME) == 0 &&
	       fsl_otp_read_by_index(8*SONOS_FUSE_UNLOCK_CTR_BANK +
			             SONOS_FUSE_UNLOCK_CTR_WORD, pValue) == 0;
}
EXPORT_SYMBOL(get_imx6_fuse);
#endif
