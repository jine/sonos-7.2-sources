...
static int enable_imx = 1;
#else
static int enable_imx;
#endif

static int __init enable_imx_setup(char *str)
{
	if(!str) return 1;

	// when uboot passes enable_imx it always passes 1
	enable_imx = simple_strtoul(str, NULL, 10);

	return 1;
}
__setup("enable_imx=", enable_imx_setup);
#endif	// CONFIG_SONOS
...
static irqreturn_t imx_rxint(int irq, void *dev_id)
{
	...
	while (readl(sport->port.membase + USR2) & USR2_RDR) {
		...
		/* Add special key ctrl+q to simulate the break key */
		if ((rx&0xff) == 0x11 )
			if (uart_handle_break(&sport->port))
				continue;
#endif
...
#ifdef CONFIG_SONOS
	pr_info("Serial: IMX driver %sabled\n", enable_imx ? "en" : "dis");
	if(!(enable_imx)) {
		return (-ENOMEM);
	}
#else
