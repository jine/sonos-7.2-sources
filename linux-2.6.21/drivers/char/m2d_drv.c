...
static unsigned int gcu_clock_counter = 0;
static spinlock_t gcu_lock = SPIN_LOCK_UNLOCKED;

/****************************************************************************
 * Clock Management of 2D GCU
 ***************************************************************************/
static void enable_gcu_clock(void)
{
	spin_lock(&gcu_lock);
	if (!gcu_clock_counter++)
		pxa_set_cken(CKEN_GRAPHICS, 1);
	spin_unlock(&gcu_lock);
}

static void disable_gcu_clock(void)
{
	spin_lock(&gcu_lock);
	if (!--gcu_clock_counter)
...
static void dump_gcu_buffi(volatile gcu_regs_t *gr, GCU_BUFFI_ADDR buff_addr)
{
	...
	printk("GC_BUFFI_S0: Base=0x%08x, Step=0x%x, Stride=%x, PF=%x\n",
		gr->gcs0br, gr->gcs0stp, gr->gcs0str, gr->gcs0pf);
	break;
	
	case GC_BUFFI_ADDR_SRC1:
	printk("GC_BUFFI_S1: Base=0x%08x, Step=0x%x, Stride=%x, PF=%x\n",
		gr->gcs1br, gr->gcs1stp, gr->gcs1str, gr->gcs1pf);
	break;
	
	case GC_BUFFI_ADDR_DST0:
	printk("GC_BUFFI_D0: Base=0x%08x, Step=0x%x, Stride=%x, PF=%x\n",
		gr->gcd0br, gr->gcd0stp, gr->gcd0str, gr->gcd0pf);
	break;
	
	case GC_BUFFI_ADDR_DST1:
	printk("GC_BUFFI_D1: Base=0x%08x, Step=0x%x, Stride=%x, PF=%x\n",
		gr->gcd1br, gr->gcd1stp, gr->gcd1str, gr->gcd1pf);
	break;
	
	case GC_BUFFI_ADDR_DST2:
	printk("GC_BUFFI_D2: Base=0x%08x, Step=0x%x, Stride=%x, PF=%x\n",
