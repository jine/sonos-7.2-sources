...
#define DEF_TIMEOUT             32 // units of about 2us
...
#else
	// XXX BT I have no idea who would think the code above is the right
	// XXX way to do a master abort.  It's not at all...
	if ((readl(_ICR(i2c)) & ICR_IUE)==0) {
		printk("i2c: can't do a master abort when the unit is not enabled\n");
		return;
	}
	writel(readl(_ICR(i2c)) | ICR_MA, _ICR(i2c));
	while (readl(_ISR(i2c)) & ISR_UB) ;
#endif

	writel(readl(_ICR(i2c)) & ~(ICR_MA | ICR_START | ICR_STOP | ICR_TB ),
	       _ICR(i2c));
}
...
#if 0 // XXX BT this is for slave mode which we don't use and even then is
      // XXX not too sensible
...
static void i2c_pxa_reset(struct pxa_i2c *i2c,int initial)
{
	pr_debug("Resetting I2C Controller Unit\n");

	/* abort any transfer currently under way */
	if (!initial) i2c_pxa_abort(i2c);

	/* reset according to 9.8 */
	writel(ICR_UR, _ICR(i2c));
#ifdef CONFIG_I2C_FAST_MODE
	writel(readl(_ICR(i2c)) | ICR_FM, _ICR(i2c));
#endif
...
static inline void i2c_pxa_stop_message(struct pxa_i2c *i2c)
{
	 ...
	 */
	writel(readl(_ICR(i2c)) & ~(ICR_STOP | ICR_ACKNAK), _ICR(i2c));
}


#if defined(CONFIG_I2C_POLLING)
static int i2c_pxa_polling_txempty(int timeout)
{
       u32 isr;
...
static int i2c_pxa_polling_rxfull(int timeout)
{
       u32 isr;
...
static int i2c_pxa_do_read(struct i2c_msg *msg, unsigned char *buf, int len)
{
       ...
       while (len--) {
               ...
               if (i2c_pxa_polling_rxfull(12000))
                       return 1;

               *buf++ = (unsigned char) IDBR & 0xff;
       }
	...
	if (ret) {
...
#if defined(CONFIG_I2C_POLLING)
       {
               ...
               for (i=0; !err && i<num; i++) {
                       ...
                       if (p->flags & I2C_M_RD)
                               err = i2c_pxa_do_read(p, p->buf, p->len);
                       else
                               err = i2c_pxa_do_write(p, p->buf, p->len);
               }
...
static void i2c_pxa_irq_txempty(struct pxa_i2c *i2c, u32 isr)
{
 ...
 again:
	...
	if (isr & ISR_BED) {
		 ...
		 */
		// XXX BT can a condition other than a NAK cause BED to set?
		// XXX documentation seems to say no.
		if (isr & ISR_ACKNAK) {
		...
		// XXX BT at this point we're still holding the bus
		// XXX so do a master abort
		i2c_pxa_abort(i2c);
...
static void i2c_pxa_irq_rxfull(struct pxa_i2c *i2c, u32 isr)
{
	...
	if ((i2c->msg->flags&I2C_M_MEP)&&(i2c->msg_ptr==1)) {
		i2c->msg->len=((i2c->msg->buf[0]&0x07)+2);
	}
...
int __init i2c_adap_pxa_init(void)
#else
...
// XXX BT on woodstock call this at the end of micco initialization because
// XXX of a power sequencing issue
