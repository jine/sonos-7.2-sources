...
static int sclhi(struct i2c_algo_bit_data *adap)
{
	...
	while (! getscl(adap) ) {	
		...
		// XXX BT on Woodstock this can get called from inside
		// the frequency/voltage changing logic which might be
		// invoked from somewhere where it's totally inappropriate
		// to call the scheduler
		// cond_resched();
		udelay(5);
	}
	DEBSTAT(printk(KERN_DEBUG "needed %ld tries\n", tries));
...
static int i2c_outb(struct i2c_adapter *i2c_adap, char c)
{
	...
	for ( i=7 ; i>=0 ; i-- ) {
		 ...
		 */
		setscl(adap, 0 );
		// XXX BT following udelay had been previously commented out
		// XXX but I'm worried that allows SDA transition very close
		// XXX to the SCL falling edge, could potentially be
		// XXX construed as a start or stop if very unlucky
		udelay(adap->udelay);
	}
	sdahi(adap);
