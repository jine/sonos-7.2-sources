...
/*****************************************************************************
 * Abstract:
 *	contains all primitive functions for OV7673
 *
...
static CI_CMU_COE_MATRIX ov7673_cRGB_sRGB_COE = {
...
/*
   static CI_CMU_COE_MATRIX ov7673_cRGB_sRGB_COE = {
   RGB_FLOAT_TO_INT(-0.06),  RGB_FLOAT_TO_INT(-0.30), RGB_FLOAT_TO_INT(1.36),
   RGB_FLOAT_TO_INT(-0.12) ,  RGB_FLOAT_TO_INT(1.32), RGB_FLOAT_TO_INT(-0.20),
   RGB_FLOAT_TO_INT(1.59),  RGB_FLOAT_TO_INT(-0.55), RGB_FLOAT_TO_INT(-0.04)
   };
 */

static CI_CMU_COE_MATRIX ov7673_cRGB_YUV_COE = {
 ...
 * OV7673 Functions
 *****************************************************************************/
int ov7673_init( p_camera_context_t camera_context )
{
	u8 cm_rev, cm_pid;
	u32 timeout;
	int status;

	printk(KERN_DEBUG "%s: 1. ckenA: 0x%x\n", __func__, CKENA);
...
int ov7673_deinit(  p_camera_context_t camera_context )
{
...
int ov7673_set_capture_format(  p_camera_context_t camera_context )
{
	CI_MP_TIMING timing;
	int status;
	...
	if (camera_context->cmu_usage == CI_CMU_OUTPUT_YUV) {
		ci_cmu_set_color_correction_coe(&ov7673_cRGB_YUV_COE);
	}

	if (camera_context->cmu_usage == CI_CMU_OUTPUT_RGB) {
		ci_cmu_set_color_correction_coe(&ov7673_cRGB_sRGB_COE);
	}
...
int ov7673_start_capture(p_camera_context_t camera_context,
		unsigned int frames)
{
	(void) camera_context;
...
int ov7673_stop_capture(  p_camera_context_t camera_context )
{
	(void) camera_context;
...
int ov7673_set_contrast( p_camera_context_t camera_context, u8 mode, u32 value)
{
	(void) camera_context;
...
int ov7673_set_exposure( p_camera_context_t camera_context, u8 mode, u32 value)
{
	(void) camera_context;
