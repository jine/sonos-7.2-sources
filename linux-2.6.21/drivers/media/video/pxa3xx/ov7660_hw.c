...
static void sensor_power_on(void)
{
	...
	pxa3xx_gpio_set_level(MFP_CIF_LO_PWDN_GPIO, GPIO_LEVEL_LOW);
}

static void sensor_power_off(void)
{
	pxa3xx_gpio_set_level(MFP_CIF_LO_PWDN_GPIO, GPIO_LEVEL_HIGH);
	pxa3xx_gpio_set_direction(MFP_CIF_LO_PWDN_GPIO, GPIO_DIR_IN);
}
...
static void sensor_power_on(void)
{
	...
	if (technology_boards & WLANCAMERA_BOARD){
		val = WLANCAMERA_board_read_max7321();
		val &= ~WLANCAMERA_BOARD_CAMERA_MASK;
		val |= 0x84;
		WLANCAMERA_board_write_max7321(val);
		val = WLANCAMERA_board_read_max7321();
	}
}
...
#define I2C_DRIVERID_OV7660   (0)
#define	OV7660_ADDRESS	0x21

struct i2c_driver ov7660_driver  =
{
	.driver = {
		.name	= "ov7660 i2c client driver",
	},
...
static unsigned short normal_i2c[] = {OV7660_ADDRESS, I2C_CLIENT_END };
I2C_CLIENT_INSMOD_1(ov7660);
		...
		int address, int kind)
{
	...
	if ((i2c_ov7660_read(new_client, OV7660_PID) != PID_OV76XX ) ||
		...
		sensor_power_off();
		printk(KERN_INFO "OV7660 not detected.\n");
		pxa_set_cken(CKEN_CAMERA, 0);
		goto ERROR1;
	} else {
		extern int ov7660_detected;
		ov7660_detected = 1;
		printk(KERN_INFO "OV7660 detected.\n");
	}
	ci_set_clock(0, 0, 2600);
	sensor_power_off();
	pxa_set_cken(CKEN_CAMERA, 0);
...
static int __init i2c_ov7660_init(void)
{
	...
	if ( (ret = i2c_add_driver(&ov7660_driver)) ) {
		printk(KERN_ERR "ov7660: Driver registration failed,"
			"module not inserted.\n");
		return ret;
	}
...
{
	if (i2c_del_driver(&ov7660_driver)) {
		printk(KERN_ERR "ov7660: Driver registration failed,"
		"module not removed.\n");
	}
}
