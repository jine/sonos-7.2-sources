...
#if defined(CONFIG_DVFM) && !defined(CONFIG_CPU_PXA310)
#define IRDA_OUT_D0CS
#else
#undef IRDA_OUT_D0CS
#endif
...
#ifdef IRDA_OUT_D0CS
static int irda_dvfm_notifier(unsigned int cmd, void* client_data, void *info);
#endif
...
static int pxa_irda_set_speed(struct pxa_irda *si, int speed)
{
	...
	case 57600:	case 115200:
		...
		if (IS_FIR(si)) {
			...
			pr_debug("si->pdata->transceiver_mode=%p\n",
...
#if defined(IRDA_OUT_D0CS) && defined(CONFIG_IPM)
...
static int pxa_irda_start(struct net_device *dev)
{
	...
	if (ACSR & 0x04000000){		/* if the CPU is in D0CS mode */
		...
		ipm_event_notify(IPM_EVENT_DEVICE, IPM_EVENT_DEVICE_OUTD0CS,
				 NULL, 0);
		ret = wait_event_interruptible_timeout(si->delay_wait, 
				(!(ACSR & 0x04000000)), HZ/100);
		
		if((ret<=0) && (ACSR & 0x04000000)){ 
			pr_debug("ret = %d\n", ret);
			return -EAGAIN;
		}
	}
...
#ifdef IRDA_OUT_D0CS
static int
irda_dvfm_notifier(unsigned int cmd, void* client_data, void *info)
{
	struct pxa3xx_fv_notifier_info *notifier_info;
	struct pxa_irda *si= (struct pxa_irda*)client_data;
	unsigned int baudrate_mask;
	unsigned int flag = 0;

	notifier_info = (struct pxa3xx_fv_notifier_info*)info;
	switch (cmd) {
	case FV_NOTIFIER_QUERY_SET :
		if(si->irlap && (notifier_info->cur.d0cs == 0 &&
		   notifier_info->next.d0cs == 1)) { 
			pr_debug("IrDA interface up, please try \
				  D0CS mode again.\n");
			return -EAGAIN;
		}
		break;
	...
	case FV_NOTIFIER_POST_SET :
		if (notifier_info->cur.d0cs == 0 &&
			...
			si->clk_src = 921600*16;
			flag = 1;
			wake_up(&si->delay_wait);
		}else
			flag = 0;
		
		if (flag){
			pxa_irda_set_speed(si, si->speed);
...
#ifdef IRDA_OUT_D0CS
	si->dvfm_notifier.name = "pxa2xx-ir";
	si->dvfm_notifier.priority = 0;
	si->dvfm_notifier.notifier_call = irda_dvfm_notifier;
	init_waitqueue_head(&si->delay_wait);
#endif
	/*
...
#ifdef IRDA_OUT_D0CS
	si->dvfm_notifier.client_data = si;
	pxa3xx_fv_register_notifier(&si->dvfm_notifier);
#endif
