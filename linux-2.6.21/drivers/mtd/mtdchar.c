...
readscrub:
        ...
        {
		void *b=0;
                struct mtd_bbop_info bbi;
                if (copy_from_user(&bbi,(void *)arg,sizeof(bbi)))
			return -EFAULT;
                if ((bbi.op!=OP_READ)&&(bbi.op!=OP_WRITE)&&(bbi.op!=OP_ERASE)) return -EINVAL;
		if ((bbi.op==OP_ERASE)&&(bbi.len!=0)) return -EINVAL;
                if ((bbi.op!=OP_ERASE)&&((bbi.len<1)||(bbi.len>131072)) ) return -EINVAL;
		if (bbi.len) {
			b=kzalloc(bbi.len,GFP_KERNEL);
			if (!b) return -ENOMEM;
		}
		if (bbi.op==OP_WRITE) {
			if (copy_from_user(b,((void *)arg)+sizeof(struct mtd_bbop_info),bbi.len)) {
				kfree(b);
				return -EFAULT;
			}
		}
		ret=mtd->bb_op(mtd,0,bbi.len,b,bbi.op);
		if (ret) {
			if (b) kfree(b);
			return ret;
		}
		if (bbi.op==OP_READ) {
			if (copy_to_user(((void *)arg)+sizeof(struct mtd_bbop_info),b,bbi.len)) {
				kfree(b);
				return -EFAULT;
			}
		}
