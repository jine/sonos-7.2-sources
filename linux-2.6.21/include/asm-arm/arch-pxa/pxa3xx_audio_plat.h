/*
 * linux/sound/arm/audio_codec_plat.h
 *
 * Author:	Xu Jingqing
 * Created:	July, 2005
 ...
 * All Rights Reserved
 */
#ifndef _AUDIO_CODEC_PLAT_ZY_H
#define _AUDIO_CODEC_PLAT_ZY_H
...
#define SUPPORT_INPUT_CONTROL(ainfo, idx)	((ainfo).support_input_control & \
		(u16)1 <<(idx))
#define SUPPORT_OUTPUT_CONTROL(ainfo, idx)	((ainfo).support_output_control &\
		(u16)1<<(idx))
#define SUPPORT_VOL_CONTROL(ainfo,idx)		((ainfo).support_vol_control &	\
		1<<(idx))
#define SUPPORT_OTHER_CONTROL(ainfo,idx)	((ainfo).support_other_control &\
		1<<(idx))
...
extern void set_card_shortname(struct snd_card *card, p_acodec_context_t p_zy_codec_ctxt);
extern int alsa_codec_regs_dump(p_acodec_context_t p_zy_codec_ctxt);
extern int alsa_prepare_for_zy(p_acodec_context_t * p_zy_codec_ctxt);
extern void alsa_zy_codec_put(p_acodec_context_t p_zy_codec_ctxt);
extern int audio_codec_zy_do_suspend(struct snd_card *card, unsigned int state, p_acodec_context_t p_zy_codec_ctxt);
extern int audio_codec_zy_do_resume(struct snd_card *card, unsigned int state, p_acodec_context_t p_zy_codec_ctxt);
