...
#ifdef IMM_SRAM_FIRST_BANK
#define IMM_SRAM_BANK_COUNT     (PHYS_SRAM_BANK_COUNT-IMM_SRAM_FIRST_BANK)
#else
#define IMM_SRAM_BANK_COUNT     (0)
#endif
#ifdef IMM_SRAM_START
#define IMM_SRAM_SIZE           (PHYS_SRAM_SIZE - (IMM_SRAM_START - PHYS_SRAM_START))
#else
#define IMM_SRAM_SIZE           (0)
#endif
#define IMM_SRAM_PAGES          (IMM_SRAM_SIZE / PAGE_SIZE)


#if defined(CONFIG_IMM_API_SRAM) && (IMM_SRAM_PAGES == 0)
#error "For SRAM Allocation to work there must at least be some SRAM available"
#endif
