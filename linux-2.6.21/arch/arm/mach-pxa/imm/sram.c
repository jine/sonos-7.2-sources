...
u32 num_free_sram_pages = SRAMEND - SRAMSTART;
...
struct imm_page_t page_list[IMM_SRAM_PAGES];
...
void print_page_list(char *s)
{
	...
	printk(KERN_DEBUG "%d pages total, %d free\n",
		&page_list[SRAMEND]-&page_list[SRAMSTART], space);
	for (count = 0, i = 0, p = &page_list[SRAMSTART]; p < &page_list[SRAMEND]; p++, i++) {
 ...
 *
 *************************************************************************/
extern void xsc3_l2_cache_allocate_and_lock(u32, u32);
extern void xsc3_l2_cache_invalidate_range(u32, u32);
...
extern void vunmap_pud_range(pgd_t *pgd, unsigned long addr,
					unsigned long end);
void imm_unmap_page(struct imm_page_t *page, struct imm_info_t *imm_info)
{
	...
	if (PAGE_IS_MAPPED(page->flag)) {
		if (IMMID_USER(imm_info->immid)) {
			...
			dir = pgd_offset_k(page->virt_addr);
			vunmap_pud_range(dir, page->virt_addr,
...
int imm_alloc_pages(u32 virt, u32 size, u32 flags, struct imm_info_t *imm_info)
{
	...
	if ((pages_needed = (virt_end-virt_start)/PAGE_SIZE) > num_free_sram_pages)
	...
	if (!PAGE_IS_DMA(flags)) {
		...
		for (p = &page_list[SRAMSTART]; (p < &page_list[SRAMEND])&&
		...
		if (page_map_ptr[0]) page_map = page_map_ptr;
		...
		for (p = &page_list[SRAMSTART], pages_found = 0;
			(p < &page_list[SRAMEND])&&(pages_found < pages_needed);
			p++)
		{
			if (!PAGE_IS_MAPPED(p->flag))
				page_map[pages_found++] = p;
...
imm_free_pages_immid(struct imm_info_t *imm_info)
{
	...
	/* first scan to pick up needed pages that are already mapped */
	for (p = &page_list[SRAMSTART]; p < &page_list[SRAMEND]; p++)
	{
		if (PAGE_IS_MAPPED(p->flag)&&
			(p->immid == immid))
		{
...
int
release_immid(struct imm_info_t *imm_info)
{
	struct imm_virt_t *head = imm_info->malloc_list,
			*p = imm_info->malloc_list->next;
	u32 immid = imm_info->immid;
...
int
page_compatible(u32 start, u32 flags, u32 immid)
{
...
void *
imm_malloc(size_t size, u32 flags, u32 immid)
{
	...
	if ((!PAGE_IS_DMA(flags) && ((size+imm_info->used_space)
		> IMM_CLIENT_MEMORY_SIZE)) || (PAGE_IS_DMA(flags)
		&&((size+imm_info->used_space)
		> min(IMM_SRAM_SIZE, IMM_CLIENT_MEMORY_SIZE)))) {
...
int
imm_free(void *address, u32 immid)
{
	int res;
	struct imm_info_t *imm_info;
...
unsigned int
imm_get_freespace(u32 flags, u32 immid) {
	struct imm_page_t *p;
	int bytes = IMM_SRAM_SIZE;
	struct imm_info_t *imm_info;
	...
	up(&imm_sem);
	return min(bytes, IMM_CLIENT_MEMORY_SIZE-imm_info->used_space);
}
...
READ_PROC_PROTO(imm_sram_usagemap_read_proc)
{
	...
	READ_PROC_INIT
	for (i = IMM_SRAM_FIRST_BANK; i < PHYS_SRAM_BANK_COUNT; i++) {
		pprintf(" ");
                for (j = 0; j < PHYS_SRAM_PAGES_PER_BANK; j++)
                        pprintf("%c", (i-IMM_SRAM_FIRST_BANK)?'-':'_');
                pprintf(" \n|");
                for (j = SRAM_ARRAY_STARTPAGE(i);
                        j < SRAM_ARRAY_STARTPAGE(i+1);
                        j++) {
                        pprintf("%c",
                                PAGE_IS_MAPPED(page_list[j].flag)?
				(IMMID_USER(page_list[j].immid)?'U':'K'):'.');
                }
                pprintf("| SRAM Bank %d\n", i);
        }
        pprintf(" ");
...
int i;
	...
	READ_PROC_INIT
	for (i = 0, p = &page_list[SRAMSTART]; p < &page_list[SRAMEND]; p++, i++) {
		if (PAGE_IS_MAPPED(p->flag)) {
...
void
imm_dau_sram_init (struct proc_dir_entry *imm_dir)
{
	...
	for (i = 0, p = &page_list[SRAMSTART]; p < &page_list[SRAMEND]; p++, i++) {
		...
		plist[i] = '.';
#endif
		p->phys_addr = SRAM_PHYS_ADDRESS(i);
