...
struct pmic_ops *pxa3xx_pmic_ops = NULL;
...
int pxa3xx_pmic_set_apps_and_sram(int apps_mv,int sram_mv)
{
	int ret;
	ret = check_pmic_ops();
	if (ret < 0)
	...
	if (pxa3xx_pmic_ops->set_apps_and_sram) {
		return pxa3xx_pmic_ops->set_apps_and_sram(apps_mv,sram_mv);
	} else {
		ret=pxa3xx_pmic_set_voltage(VCC_CORE,apps_mv);
		if (ret < 0) return ret;
		return pxa3xx_pmic_set_voltage(VCC_SRAM,sram_mv);
	}
}
