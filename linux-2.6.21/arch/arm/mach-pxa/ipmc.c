...
static void cop_event_handler(prm_event event, unsigned int group_id, void *data)
{
...
int ipmc_open(struct inode *inode, struct file *filep)
{
	...
	ret = prm_open_session(IPMC_CLI_PRI, IPMC_CLI_NAME, cop_event_handler, (void *)NULL);
	if (ret < 0){
	...
	if ((ret = prm_allocate_resource(ipmc_client_id, PRM_COP, GROUP_COP))) {
	...
	if (prm_commit_resources(ipmc_client_id, GROUP_COP)){
		...
		wait_event_interruptible(cop_wait_q,
			(rsrcs_avail != 0));
		...
		/* init DPM */
		dynamicpower_init();
		/* ipm policy maker would set DPM op and policy */
#endif
	}
		...
		loff_t *f_pos)
{
	...
	retval = copy_to_user((struct ipm_event *)buf, &data, sizeof(struct ipm_event));
	if (retval) {
		retval = -EFAULT;
		goto out;
	}
		...
		unsigned long arg)
{
	...
	case IPMC_IOCTL_GET_FV_OP:
	{
		...
		if ((ret = cop_get_cur_cop(ipmc_client_id, &op, &param)))
			break;

		if (copy_to_user((unsigned long *)arg, &op, sizeof(op))){
			ret = -EFAULT;
			break;
		}
		break;
	...
	case IPMC_IOCTL_SET_FV_OP:
	{
		...
		if (copy_from_user(&setop, (unsigned int *)arg, sizeof(setop))) {
			printk(KERN_ERR "Error from copy_from_user.\n");
			ret = -EFAULT;
			break;
		}
		ret = cop_set_cop(ipmc_client_id, setop.op, setop.mode);
	...
	case IPMC_IOCTL_GET_FV_OP_COUNT:
	{
		...
		if (copy_to_user((unsigned long *)arg, &count, sizeof(count)))
			ret = -EFAULT;
		break;
	}
	case IPMC_IOCTL_GET_FV_OP_INFO:
	{
		...
		if (copy_from_user(&getinfo, (unsigned int *)arg, sizeof(getinfo))) {
			printk(KERN_ERR "Error from copy_from_user.\n");
			ret = -EFAULT;
			break;
		}
		if ((ret = cop_get_cop(ipmc_client_id, getinfo.op, &(getinfo.info))))
			break;
		if (copy_to_user((unsigned long *)arg, &getinfo, sizeof(getinfo)))
			ret = -EFAULT;
		break;
	}

	case IPMC_IOCTL_SET_CPU_MODE:
		if (copy_from_user(&cpu_mode, (unsigned int *)arg, sizeof(unsigned int))){
			ret = -EFAULT;
			break;
		}
		pm_suspend(cpu_mode);
	...
	case IPMC_IOCTL_SET_SLEEPTIME:
		if (copy_from_user(&pm_sleeptime, (unsigned int *)arg, sizeof(unsigned int)) )
			ret = -EFAULT;
		break;

	case IPMC_IOCTL_GET_SLEEPTIME:
		if (copy_to_user((unsigned int *)arg, &pm_sleeptime, sizeof(unsigned int)))
			ret = -EFAULT;
		break;
	...
	case IPMC_IOCTL_SET_WAKETIME:
		if (copy_from_user(&pm_waketime, (unsigned int *)arg, sizeof(unsigned int)) )
			ret = -EFAULT;
		if (pm_waketime != 0)		/* Wakeup Timeout can't be 0 */
			pm_updatetimer(IPM_WAKEUPTIMER);
		break;

	case IPMC_IOCTL_GET_WAKETIME:
		if (copy_to_user((unsigned int *)arg, &pm_waketime, sizeof(unsigned int)))
			ret = -EFAULT;
		break;

	case IPMC_IOCTL_SET_UITIME:
		if (copy_from_user(&pm_uitimeout, (unsigned int *)arg, sizeof(unsigned int)) ){
			ret = -EFAULT;
			break;
		}
		if (pm_uitimeout != 0)		/* UI Timeout can't be 0 */
			pm_updatetimer(IPM_UITIMER);
		break;

	case IPMC_IOCTL_GET_UITIME:
		if (copy_to_user((unsigned int *)arg, &pm_uitimeout, sizeof(unsigned int)))
			ret = -EFAULT;
		break;
	...
	case IPMC_IOCTL_STARTPMU:
	{
		...
		if (copy_from_user(&len, (unsigned int *)arg, sizeof(unsigned int))) {
			ret = -EFAULT;
			break;
		}
		if (len > 0) {
			if ((pmu_arg = kmalloc(len, GFP_KERNEL)) == NULL) {
				printk(KERN_ERR "can't allocate enough memory\n");
				ret = -ENOMEM;
				break;
			}
			if (copy_from_user(pmu_arg, (unsigned int *)arg, len)) {
				ret = -EFAULT;
				kfree(pmu_arg);
				break;
			}
	...
	case IPMC_IOCTL_SET_WAKEUPSRC:
		if (copy_from_user(&wakeup_src, (pm_wakeup_src_t *)arg, sizeof(pm_wakeup_src_t)))
			ret = -EFAULT;
		break;

	case IPMC_IOCTL_GET_WAKEUPSRC:
		if (copy_to_user((pm_wakeup_src_t *)arg, &wakeup_src, sizeof(pm_wakeup_src_t)))
			ret = -EFAULT;
		break;
#endif

	case IPMC_IOCTL_SET_DEEPIDLE:
		if (copy_from_user(&enable_deepidle, (unsigned int *)arg, sizeof(unsigned int)) )
			ret = -EFAULT;
		break;
	...
	if (ret == EACCES || !rsrcs_avail){
        	/* if PRM resources are appropriated, let IPM policy maker thread enter sleep */
	        DPRINTK("PRM resources are appropriated, enter sleep...\n");
        	rsrcs_avail = 0;
#ifdef CONFIG_DPM
		/* disable DPM */
