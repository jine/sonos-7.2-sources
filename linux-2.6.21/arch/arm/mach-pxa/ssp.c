...
void *ssp_rcv_callback_arg=0;
void (*ssp_rcv_callback)(void *)=0;
#endif
...
#ifdef CONFIG_MACH_WOODSTOCK
	if ((ssp_rcv_callback)&&(status & SSSR_RFS))
		ssp_rcv_callback(ssp_rcv_callback_arg);
#endif
...
#ifdef CONFIG_MACH_WOODSTOCK
int ssp_rcv_ready(struct ssp_dev *dev)
{
	return !!(SSSR_P(dev->port) & SSSR_RNE);
}
#endif
...
#ifdef CONFIG_MACH_WOODSTOCK
EXPORT_SYMBOL(ssp_rcv_ready);
EXPORT_SYMBOL(ssp_rcv_callback);
EXPORT_SYMBOL(ssp_rcv_callback_arg);
#endif
