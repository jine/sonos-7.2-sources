...
#ifdef CONFIG_NO_IDLE_HZ
int pxa_dyn_enable = 0;
static int pxa_dyn_tick_enable(void)
{
	pxa_dyn_enable = 1;
	return 0;
}

static int pxa_dyn_tick_disable(void)
{
	pxa_dyn_enable = 0;
	return 0;
}

extern void ipm_deepidle(unsigned long ticks);
static void pxa_dyn_tick_reprogram(unsigned long ticks)
{
	if (match_posponed) {
		printk("pxa_dyn_tick_reprogram called with match_posponed set, expect badness!\n");
	}
	if (ticks > 1) {
		initial_match = OSMR0;
...
static struct dyn_tick_timer pxa_dyn_tick = {
	.enable		= pxa_dyn_tick_enable,
	.disable	= pxa_dyn_tick_disable,
