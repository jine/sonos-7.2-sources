...
#define CMDLINE_APPEND_STRING "append:"
#define CMDLINE_APPEND_LEN    7
#endif

static int __init parse_tag_cmdline(const struct tag *tag)
{
#ifdef CONFIG_MACH_WOODSTOCK
	if (0 == strncmp(tag->u.cmdline.cmdline, CMDLINE_APPEND_STRING, CMDLINE_APPEND_LEN)) {
		strncat(default_command_line, &(tag->u.cmdline.cmdline[CMDLINE_APPEND_LEN]), COMMAND_LINE_SIZE); 
	} else {
...
static int __init parse_tag_mdp(const struct tag *tag)
{
	memcpy(&sys_mdp,&(tag->u.mdp.mdp),sizeof(sys_mdp));
	return 0;
}
__tagtable(ATAG_MDP, parse_tag_mdp);
