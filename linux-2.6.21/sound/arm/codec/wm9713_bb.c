...
acodec_error_t zy_wm9713_set_master_vol(acodec_context_t *p_device_context, unsigned short gain_in_db)
{
	...
	{
		left_volume = ZY_AC97_WM9713_MAX_VOLUME - left_volume;	/* translate into attenuation value */
...
acodec_error_t zy_wm9713_set_voice_out_sample_rate(acodec_context_t *p_device_context, unsigned short rate_in_hz)
{
	...
	/* reg 0x44 DIV, the PCM_CLK= AC97_CLK/DIV, the PCM_CLK should be
	 * 24.576 Mhz. however, basing on the test, PCM_CLK = AC97_CLK/(DIV*2)
	 */
	switch (rate_in_hz)
	{
		case 8000:
...
static acodec_error_t  zy_wm9713_hifi_near_in_enable(acodec_context_t *p_device_context)
{
	 ...
	 * 3eh to control specific power domain 3CH  bit 7 and 6, power on
	 * left DAC and right DAC
	 */
...
acodec_error_t	zy_wm9713_hifi_stream_path_disable(acodec_context_t *p_device_context)
{
	...
	/* mute DAC */
	zy_ac97_acodec_write(p_device_context, DAC_PGA_VOL_ROUTE, 0xe808);	/*0c*/
	/* power DAC off, we don't change 26h setting. insteadly we use 3ch and 3eh to control specific power domain 3CH  bit 7 and 6 */
	status = zy_ac97_acodec_read(p_device_context, POWER_DOWN_1, &value);
	value |= (ZY_AC97_9713_PWR_DACL | ZY_AC97_9713_PWR_DACR);
	status = zy_ac97_acodec_write(p_device_context, POWER_DOWN_1, value);
...
acodec_error_t	zy_wm9713_headset_enable(acodec_context_t *p_device_context)
{
	...
	/* mixer route 0ch:15, this bit control belongs to hifipath */
	/* to headset mixer */
	...
	/* power on left/right headpone mix */
	status = zy_ac97_acodec_read(p_device_context, POWER_DOWN_1, &value);
...
/* static acodec_state_t wm9713_codec_state =CODEC_SLEEP; */
...
static acodec_error_t zy_wm9713_manage_mic_in(
					acodec_context_t *p_device_context,
					unsigned short mic1_in,
					unsigned short mic2_in
					)
{
	...
	if(!mic2_in){
		 zy_wm9713_mic_disable(p_device_context, HEADSET_MIC_MASK);
	}

	return ACODEC_SUCCESS;
}

static void set_ssp_for_bb(acodec_context_t *p_device_context, int if_open)
{
	static int bb_ssp_state_on = 0;
	static unsigned short mclk_pll_ctrl = 0;
	static unsigned short pcm_codec_ctrl = 0;
	static unsigned short value = 0;
	if((if_open == 1) && (!bb_ssp_state_on) ){
		printk("open SSP for BB \n");
			zy_ac97_acodec_read(p_device_context, MCLK_PLL_CTRL_1, &value);
		mclk_pll_ctrl = value;
		printk("MCLK_PLL_CTRL_1:0x%x\n", value);
		value = 0xf80;
		zy_ac97_acodec_write(p_device_context, MCLK_PLL_CTRL_1, value);

		zy_ac97_acodec_read(p_device_context, PCM_CODEC_CTRL, &value);
		pcm_codec_ctrl = value;
		value = 0xa8b3;
		zy_ac97_acodec_write(p_device_context, PCM_CODEC_CTRL, value);
		bb_ssp_state_on = 1;
	}
	if((if_open == 0) && bb_ssp_state_on ){
		printk("close SSP for BB \n");

		zy_ac97_acodec_write(p_device_context, MCLK_PLL_CTRL_1, mclk_pll_ctrl);
				zy_ac97_acodec_write(p_device_context, PCM_CODEC_CTRL, pcm_codec_ctrl);

		bb_ssp_state_on = 0;
	}
}
static far_in_on = 0;
static void zy_open_far_in(acodec_context_t *p_device_context)
{
	unsigned short value=0;
	/* power on VXDAC */
	zy_ac97_acodec_read(p_device_context, POWER_DOWN_1, &value);
	...
	zy_ac97_acodec_write(p_device_context, POWER_DOWN_1, value);
	/*set ssp for BB*/
	set_ssp_for_bb(p_device_context, 1);
	far_in_on = 1;
#if 0
	/* set SSP */
	printk("set SSP for BB in zy_open_far_in");
	zy_ac97_acodec_read(p_device_context, MCLK_PLL_CTRL_1, &value);
	value = 0xf80;
	zy_ac97_acodec_write(p_device_context, MCLK_PLL_CTRL_1, value);
...
static void zy_close_far_in(acodec_context_t *p_device_context)
{
	unsigned short value=0;
	/* power off VXDAC */
	zy_ac97_acodec_read(p_device_context, POWER_DOWN_1, &value);
...
static acodec_error_t zy_wm9713_manage_far_in(
				acodec_context_t *p_device_context,
				unsigned short *rout_map,
				unsigned short* current_map
				)
{
	unsigned short far_in, far_in_change_state;
	far_in = rout_map[INDEX_FAR_IN_CONTROL];
	far_in_change_state = (!!rout_map[INDEX_FAR_IN_CONTROL])
				 ^ (!!current_map[INDEX_FAR_IN_CONTROL]);
	if(far_in_change_state){
		if(far_in) {
			/* open FAR IN */
			zy_open_far_in(p_device_context);
		}else{
			/* close FAR IN */
			zy_close_far_in(p_device_context);
		}
	}
...
static acodec_error_t zy_wm9713_manage_near_in(
					acodec_context_t *p_device_context,
					unsigned short *rout_map,
					unsigned short* current_map
					)
{
	unsigned short hifi_near_in_change_state;
	unsigned short voice_near_in_change_state;

	unsigned short hifi_near_in = rout_map[INDEX_HIFI_NEAR_IN_CONTROL];
	unsigned short voice_near_in = rout_map[INDEX_VOICE_NEAR_IN_CONTROL];
	...
	if(hifi_near_in_change_state){
		if(current_map[INDEX_HIFI_NEAR_IN_CONTROL] && ! hifi_near_in) {
			/* from on to off */
			zy_wm9713_close_hifi_near_in(p_device_context);
		}else if(!current_map[INDEX_HIFI_NEAR_IN_CONTROL] && hifi_near_in )
		{
			/* from off to on */
	...
	if(voice_near_in_change_state){
		if(current_map[INDEX_VOICE_NEAR_IN_CONTROL] && ! voice_near_in){
			/* from on to off */
			zy_wm9713_close_voice_near_in(p_device_context);
		}else if(!current_map[INDEX_VOICE_NEAR_IN_CONTROL] && voice_near_in )
		{
			/* from off to on */
...
static acodec_error_t zy_wm9713_manage_out(
				acodec_context_t *p_device_context,
				unsigned short *rout_map,
				unsigned short* current_map
			)
{
	unsigned short out_state, current_out_state, out_change_state;

	out_state =
		rout_map[INDEX_HIFI_NEAR_IN_CONTROL] |
		rout_map[INDEX_MIC1_IN_CONTROL]|
		rout_map[INDEX_MIC2_IN_CONTROL]|
		rout_map[INDEX_VOICE_NEAR_IN_CONTROL]|
		rout_map[INDEX_FAR_IN_CONTROL];
	if( out_state & FAR_OUT_ROUTE){
		set_ssp_for_bb(p_device_context, 1);
#if 0
		unsigned short value;
		/* set SSP */
		printk("set SSP for BB in manage out!\n");
		zy_ac97_acodec_read(p_device_context, MCLK_PLL_CTRL_1, &value);
		value = 0xf80;
		zy_ac97_acodec_write(p_device_context, MCLK_PLL_CTRL_1, value);
		...
		if(!far_in_on){
		/*FAR IN and OUT all off, so close the ssp for BB*/
			set_ssp_for_bb(p_device_context, 0);
		}
	}



	/* The HIFI NEAR OUT and FAR NEAR OUT is the same output device on wm9713 */
	/* Because the LDAC and RDAC will send the analog signal both to PCM and */
	/* AC97 */
	out_state |= (!!(out_state & FAR_OUT_ROUTE)) << INDEX_HIFI_NEAR_OUT_CONTROL;


	current_out_state =
		current_map[INDEX_HIFI_NEAR_IN_CONTROL]|
		current_map[INDEX_MIC1_IN_CONTROL] |
		current_map[INDEX_MIC2_IN_CONTROL]|
		current_map[INDEX_VOICE_NEAR_IN_CONTROL]|
		current_map[INDEX_FAR_IN_CONTROL];
	current_out_state |=
		(!!(current_out_state & FAR_OUT_ROUTE)) <<
...
*/
	...
	if(out_change_state & (HIFI_NEAR_OUT_ROUTE | VOICE_NEAR_OUT_ROUTE) ) {
		if((current_out_state & (HIFI_NEAR_OUT_ROUTE | VOICE_NEAR_OUT_ROUTE))
			&&
		!( out_state & (HIFI_NEAR_OUT_ROUTE | VOICE_NEAR_OUT_ROUTE))){
			/* change from open to close */
...
}/* end of zy_wm9713_manage_out() */

static acodec_error_t zy_wm9713_manage_path(
				acodec_context_t *p_device_context,
				unsigned short *rout_map,
				unsigned short* current_map
				  )
{
	...
	unsigned short far_in, mic1_in, mic2_in, hifi_near_in, voice_near_in, val;
	...
	if((mic1_in | mic2_in) && (HIFI_NEAR_OUT_ROUTE | FAR_OUT_ROUTE))
	{
	...
	if(far_in & STEREO_HEAD_SET_OUT_ROUTE){
		/* open the path from far_in to headset */
		unsigned short value = 0;
		zy_ac97_acodec_read(p_device_context, VXDAC_VOLUME_ROUTE, &value);
		value &= ~(0xf000);
		zy_ac97_acodec_write(p_device_context, VXDAC_VOLUME_ROUTE, value);
		/* default to set the sample rate to 8k HZ for the LRP team */
		printk("set default sample rate for BB\n");
		zy_wm9713_set_out_sample_rate(p_device_context, 8000);
		zy_wm9713_set_in_sample_rate(p_device_context,8000);
	}else{
		/* close path from far_in to headset */
		unsigned short value = 0;
		zy_ac97_acodec_read(p_device_context, VXDAC_VOLUME_ROUTE, &value);
		value |= 0x8000;
		zy_ac97_acodec_write(p_device_context, VXDAC_VOLUME_ROUTE, value);
...
{
	unsigned short  mic2_in, mic1_in;
...
*/
	mic1_in = rout_map[INDEX_MIC1_IN_CONTROL];
	mic2_in = rout_map[INDEX_MIC2_IN_CONTROL];
	zy_wm9713_manage_mic_in(p_device_context, mic1_in, mic2_in);

	zy_wm9713_manage_near_in(p_device_context, rout_map, current_map);

	zy_wm9713_manage_far_in(p_device_context, rout_map, current_map);

	/* manage ouput device */
	zy_wm9713_manage_out(p_device_context, rout_map, current_map);

	/* manage path */
	zy_wm9713_manage_path(p_device_context, rout_map, current_map);
