...
int golden_muted = 0;
extern struct snd_pcm_ops pseudo_pcm_ops;
	...
	struct snd_pcm_hw_params *params)
{
	...
	if(golden_muted)
		return pseudo_pcm_ops.hw_params(substream, params);
...
static int pxa3xx_pcm_hw_free(struct snd_pcm_substream *substream)
{
	struct pxa3xx_runtime_data *prtd = substream->runtime->private_data;
	if(golden_muted)
		return pseudo_pcm_ops.hw_free(substream);
...
static int pxa3xx_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	if(golden_muted)
		return pseudo_pcm_ops.prepare(substream);
...
static int pxa3xx_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	if(golden_muted)
		return pseudo_pcm_ops.trigger(substream, cmd);
...
pxa3xx_pcm_pointer(struct snd_pcm_substream *substream)
{
	...
	if(golden_muted)
		return pseudo_pcm_ops.pointer(substream);
...
static int pxa3xx_pcm_open(struct snd_pcm_substream *substream)
{
	...
	if(golden_muted)
		return pseudo_pcm_ops.open(substream);
...
static int pxa3xx_pcm_close(struct snd_pcm_substream *substream)
{
	...
	if(golden_muted)
		return pseudo_pcm_ops.close(substream);
	...
	struct vm_area_struct *vma)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	if(golden_muted)
		return pseudo_pcm_ops.mmap(substream, vma);
...
int pxa3xx_pcm_new(struct snd_card *card, struct snd_soc_codec_dai *dai,
	struct snd_pcm *pcm)
{
	int ret = 0;
