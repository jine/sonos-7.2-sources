...
static void console_init(void)
{
	...
	if ((s = getenv("CONSOLE")) != NULL || (s = getenv("console")) != NULL) {
        if (s[0]!='/') {
            safe_strncpy(console,"/dev/",sizeof(console));
            safe_strncpy(console+5,s,sizeof(console)-5);
        } else {
		    safe_strncpy(console, s, sizeof(console));
        }
        message(LOG|CONSOLE,"busybox init: try console %s\n",console);
#if #cpu(sparc)
	 ...
	 * passed to init, so catch it here */
		...
		if (ioctl(0, TIOCGSERIAL, &sr) == 0) {
			...
			safe_strncpy(console, "/dev/tts/0", sizeof(console));
			/*tried++;*/
		}
	}
